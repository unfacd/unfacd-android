...
public final class OptionalUtilTest {
  ...
  public void absent_are_equal() {
    assertTrue(OptionalUtil.byteArrayEquals(Optional.<byte[]>empty(), Optional.<byte[]>empty()));
  }

  @Test
  public void first_non_absent_not_equal() {
    assertFalse(OptionalUtil.byteArrayEquals(Optional.of(new byte[1]), Optional.<byte[]>empty()));
  }

  @Test
  public void second_non_absent_not_equal() {
    assertFalse(OptionalUtil.byteArrayEquals(Optional.<byte[]>empty(), Optional.of(new byte[1])));
  }
  ...
  public void hash_code_absent() {
    assertEquals(0, OptionalUtil.byteArrayHashCode(Optional.<byte[]>empty()));
  }
