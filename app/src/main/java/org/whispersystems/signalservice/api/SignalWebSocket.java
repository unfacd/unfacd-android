...
public final class SignalWebSocket {
  ...
  private static final String SERVER_DELIVERED_TIMESTAMP_HEADER = "X-Unfacd-Timestamp";
...
//        getUnidentifiedWebSocket().sendKeepAlive();
  ...
  public Single<WebsocketResponse> request(WebSocketRequestMessage requestMessage, Optional<List<String>> headers) {//AA+ headers
    try {
      return getWebSocket().sendRequest(requestMessage, headers);
    } catch (IOException e) {
      return Single.error(e);
    }
  }

  public Single<WebsocketResponse> request(WebSocketRequestMessage requestMessage, Optional<UnidentifiedAccess> unidentifiedAccess, Optional<List<String>> headers) {//AA+ headers
    if (unidentifiedAccess.isPresent()) {
//      WebSocketRequestMessage message = WebSocketRequestMessage.newBuilder(requestMessage)//AA-
//              .addHeaders("Unidentified-Access-Key:" + Base64.encodeBytes(unidentifiedAccess.get().getUnidentifiedAccessKey()))
//              .build();
      try {
        return getUnidentifiedWebSocket().sendRequest(requestMessage, headers)//AA+ requestMessage. headers
                                          .flatMap(r -> {
                                            if (r.getStatus() == 401) {
                                              return request(requestMessage, headers);
                                            }
                                            return Single.just(r);
                                          })
                                          .onErrorResumeNext(t -> request(requestMessage, headers));//AA+ getWebSocket (as opposed to getUnidentifiedWebSocket) requestMessage instead of rebuilt 'message'
      } catch (IOException e) {
        return Single.error(e);
      }
    } else {
      //AA+
      Optional<List<String>> headersAmended = Optional.of(new LinkedList<>());
      if (!headers.isPresent())  {
        if (unidentifiedAccess.isPresent()) {
          headersAmended.get().add("Unidentified-Access-Key:" + Base64.encodeBytes(unidentifiedAccess.get().getUnidentifiedAccessKey()));
        }

        if (!TextUtils.isEmpty(TextSecurePreferences.getUfsrvCookie(ApplicationContext.getInstance()))) {
          headersAmended.get().add("X-Ufsrv-Cookie:" + TextSecurePreferences.getUfsrvCookie(ApplicationContext.getInstance()));
        }
      } else {
        headersAmended = headers;
      }
      //

      return request(requestMessage, headersAmended);
    }
  }
          ...
          throws TimeoutException, WebSocketUnavailableException, IOException
  {
    while (true) {
      ...
      WebSocketProtos.WebSocketMessage wsm  = getWebSocket().readRequest(timeout);//AA+ we retunr wsm instead of original 'WebSocketRequestMessage'

      //WebSocketResponseMessage returnResponse = createWebSocketResponse(request);//AA- replaced by block below
      if (wsm.getType().getNumber() == WebSocketProtos.WebSocketMessage.Type.RESPONSE_VALUE) { //ufs semantic: currently, response is only set for fence state sync and location updates
      ...
      try {
        ...
        if (isSignalServiceEnvelope(wsm)) {
          ...
          callback.onMessage(wsm, envelope);
          return Optional.of(envelope);
        } /*else if (isSocketEmptyRequest(request)) {//AA- not implemented
          return Optional.empty();
        }*/
       else {
        Log.d(TAG, ">>> read: request received is not of Envelop type...");
        callback.onMessage(wsm, null);
        return Optional.empty();
        }
    } finally {
        if (returnResponse != null) getWebSocket().sendResponse(returnResponse); //AA+
      }
    }
  }
  ...
  //AA+ arg
  private static Optional<String> findHeader(WebSocketProtos.WebSocketMessage message, String targetHeader) {
    if (message.getHeadersCount() == 0) {
      return Optional.empty();
    }
  ...
  {
    return  (webSocket != null && !webSocket.isDead());
  }
  ...
  public interface MessageReceivedCallback {
    void onMessage(WebSocketProtos.WebSocketMessage wsm, SignalServiceEnvelope envelope);//AA+ wsm
  }
}