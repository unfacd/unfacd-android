...
import org.whispersystems.signalservice.internal.push.http.UfsrvPendingCookieAuthorizationHeader;
...
public final class KeyBackupService {
  ...
  private static final String TAG = Log.tag(KeyBackupService.class);
          ...
          throws IOException
  {
    ...
    return new Session(authorization, currentToken != null ? currentToken : new TokenResponse(new byte[32], new byte[32], 0));//AA+ mock
  }
  ...
  private class Session implements RestoreSession, PinChangeSession {
            ...
            throws UnauthenticatedResponseException, IOException, KeyBackupServicePinException, KeyBackupSystemNoDataException
    {
      ...
      while (true) {
        ...
        try {
          KbsForUfsrv kbs = pushServiceSocket.verifyRegistrationLockV2(JsonUtil.toJson(new KbsForUfsrv(null, hashedPin.whole())), UfsrvPendingCookieAuthorizationHeader.getHeaders(TextSecurePreferences.getUfsrvPendingCookie(ApplicationDependencies.getApplication())));
          return new KbsPinData(new MasterKey(kbs.getMasterkey()), new TokenResponse(new byte[32], new byte[32], 0));
        } catch (IOException x) {
    ...
    public KbsPinData setPin(HashedPin hashedPin, MasterKey masterKey) throws IOException, UnauthenticatedResponseException {
     /* KbsData       newKbsData    = hashedPin.createNewKbsData(masterKey);
      ...
      return setPinForUfsrv(hashedPin, masterKey);//AA+
    }

    //AA+
    private KbsPinData setPinForUfsrv(HashedPin hashedPin, MasterKey masterKey) throws IOException, UnauthenticatedResponseException {
      KbsData       newKbsData    = hashedPin.createNewKbsData(masterKey); //masterkey encrypted
      KbsForUfsrv kbsForUfsrv = new KbsForUfsrv(masterKey.serialize(), hashedPin.whole(), newKbsData.getCipherText());
      pushServiceSocket.setKbsForUfsr(kbsForUfsrv);
      return new KbsPinData(masterKey, new TokenResponse(new byte[32], new byte[32], 0));//AA+ mock
    }
  ...
  //AA+
  static public final class KbsForUfsrv
  {
    public KbsForUfsrv(byte[] masterKey, byte[] argon2Hash, byte[] cipherText)
    {
    ...
    public KbsForUfsrv(byte[] masterKey, byte[]argon2Hash)
    {
     this(masterKey, argon2Hash, null);
    }
...
//    @JsonDeserialize(using = HexDeserializer.class)
    ...
    private byte[] cipherText; //AA see hashedPin.createNewKbsData(masterKey);
