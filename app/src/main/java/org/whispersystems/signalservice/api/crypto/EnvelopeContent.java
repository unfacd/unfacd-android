...
public interface EnvelopeContent {
   ...
   */
  //AA+ as per above
  static EnvelopeContent encrypted(byte[] content, ContentHint contentHint, Optional<byte[]> groupId) {
  ...
  class Encrypted implements EnvelopeContent {
    ...
    private final byte[]          content; //AA+ byte[]
    private final ContentHint      contentHint;
    private final Optional<byte[]> groupId;

    public Encrypted(byte[] content, ContentHint contentHint, Optional<byte[]> groupId) {//AA+ byte[]
            ...
            throws UntrustedIdentityException, InvalidKeyException
    {
      ...
      CiphertextMessage                message          = sessionCipher.encrypt(transportDetails.getPaddedMessageBody(content));//AA- content.toByteArray()
...
//      return Optional.of(content);
      return Optional.empty();//AA todo SenderKey no support for Content object
    }

    //AA+
    @Override
    public byte[] getUnpaddedMessage() {
      return content;
    }
  }
