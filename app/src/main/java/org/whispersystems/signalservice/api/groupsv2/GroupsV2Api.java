...
public class GroupsV2Api {
   ...
   */
  public GroupsV2AuthorizationString getGroupsV2AuthorizationString(
          ACI self,
          int today,
          ...
          throws IOException
  {
    ...
    if (false && newGroup.getAvatar().isPresent()) {//AA+ false
  ...
  //AA+ simply decrypt the group that we have just encrypted. same as above, but ignore the server uplink
  public DecryptedGroup getGroupSelfFeeding(Group group, GroupSecretParams groupSecretParams,
                                 GroupsV2AuthorizationString authorization)
          throws IOException, InvalidGroupStateException, VerificationFailedException
  {
    return groupsOperations.forGroup(groupSecretParams)
...
//      GroupJoinInfo                      joinInfo        = socket.getGroupJoinInfo(password, authorization);
      FenceInfo                          fenceInfo       = socket.getGroupJoinInfo(password, authorization);
      //AA+ use json model
      GroupJoinInfo joinInfo = GroupJoinInfo.newBuilder()
                                            .setTitle(ByteString.copyFromUtf8(fenceInfo.getJsonModel().getDname()))
                                            .setAvatar(fenceInfo.getJsonModel().getAvatar())
                                            .setMemberCount(fenceInfo.getJsonModel().getMembersCount())
                                            .setRevision(fenceInfo.getJsonModel().getEid())
//                                            .setAddFromInviteLink(fenceInfo.getJsonModel().getGroupMembershipPermission()? AccessControl.AccessRequired.ANY : AccessControl.AccessRequired.ADMINISTRATOR)
                                            .setAddFromInviteLink(AccessControl.AccessRequired.ADMINISTRATOR)//set for local testing . enable above when tested
                                            .build();
      GroupsV2Operations.GroupOperations groupOperations = groupsOperations.forGroup(groupSecretParams);
