...
public final class ReactWithAnyEmojiViewModel extends ViewModel {
  ...
  void onEmojiSelected(@NonNull String emoji) {
    if (messageId > 0) {
      ...
      repository.addEmojiToMessage(emoji, new MessageId(messageId, isMms), fid);//AA+
    }
  }
  ...
  static class Factory implements ViewModelProvider.Factory {
    ...
    Factory(@NonNull ReactWithAnyEmojiRepository repository, long messageId, boolean isMms, long fid) {//AA+ fid
      this.repository = repository;
      this.messageId  = messageId;
      this.isMms      = isMms;
    ...
    public @NonNull <T extends ViewModel> T create(@NonNull Class<T> modelClass) {
      ...
      return modelClass.cast(new ReactWithAnyEmojiViewModel(repository, messageId, isMms, new EmojiSearchRepository(ApplicationDependencies.getApplication()), fid));//AA+
    }
  }
