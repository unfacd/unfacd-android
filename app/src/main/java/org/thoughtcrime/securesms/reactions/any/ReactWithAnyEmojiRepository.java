...
final class ReactWithAnyEmojiRepository
{
  ...
  /*void addEmojiToMessage(@NonNull String emoji, @NonNull MessageId messageId, long fid)
  {//AA+
    SignalExecutors.BOUNDED.execute(() -> {
      try {
        ...
        if (oldRecord != null && oldRecord.getEmoji().equals(emoji)) {
          ...
          MessageSender.sendNewReaction(context, messageRecord.getId(), messageRecord.isMms(), emoji, fid);//AA+
  ...
  void addEmojiToMessage(@NonNull String emoji, @NonNull MessageId messageId, long fid) {
    SignalExecutors.BOUNDED.execute(() -> {
      ...
      if (oldRecord != null && oldRecord.getEmoji().equals(emoji)) {
        ...
        MessageSender.sendNewReaction(context, messageId, emoji, fid);//AA+
        ThreadUtil.runOnMain(() -> recentEmojiPageModel.onCodePointSelected(emoji));
      }
    });
  }
