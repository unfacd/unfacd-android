                                                         ...
                                                         @NonNull Callback callback)
  {
    SignalExecutors.UNBOUNDED.execute(() -> {
      try {
        ...
        Optional<GroupDatabase.GroupRecord> group          = SignalDatabase.groups()
                .getGroup(groupId);
        ...
        if (group.isPresent()) {
          ...
          DecryptedGroupJoinInfo joinInfo    = GroupManager.getGroupJoinInfoFromServer(context, groupMasterKey, groupInviteLinkUrl.getPassword(), groupInviteLinkUrl.getFid());//AA+
