...
public class ApplicationDependencyProvider implements ApplicationDependencies.Provider {
  ...
  public @NonNull SignalServiceMessageSender provideSignalServiceMessageSender(@NonNull SignalWebSocket signalWebSocket, @NonNull SignalServiceDataStore protocolStore) {
    return new SignalServiceMessageSender(provideSignalServiceNetworkAccess().getConfiguration(),
                                          ...
                                          SignalExecutors.newCachedBoundedExecutor("unfacd-messages", 1, 16, 30),//AA+
  ...
  private static class DynamicCredentialsProvider implements CredentialsProvider {
    ...
    public String getUser() {
      String user = TextSecurePreferences.getUfsrvUserId(ApplicationContext.getInstance());
      if (TextUtils.isEmpty(user)) {
        user = TextSecurePreferences.getUfsrvUsername(ApplicationContext.getInstance()); //only applicable at registration pending
      }
      return user;
    }
    ...
    public String getE164() {
      return  SignalStore.account().getUsername();//AA+
    }
    ...
    public String getSignalingKey() {
      return TextSecurePreferences.getSignalingKey(ApplicationContext.getInstance());
    }

    //AA+
    @Override
    public String getCookie() {return TextSecurePreferences.getUfsrvCookie(ApplicationContext.getInstance());}
