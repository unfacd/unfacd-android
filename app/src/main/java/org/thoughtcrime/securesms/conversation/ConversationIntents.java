...
public class ConversationIntents {
  ...
  //AA+
  private static final String GROUP_FID_EXTRA         = "group_fid";
  private static final String CHALLENGE_EXTRA         = "challenge";
  private static final String GROUP_PRIVACY_MODE      = "group_privacy_mode";//private,public
  ...
  final static class Args {
    ...
    static Args from(@NonNull Intent intent) {
      ...
      return new Args(RecipientId.from(Objects.requireNonNull(intent.getStringExtra(EXTRA_RECIPIENT))),
                      ...
                      intent.getLongExtra(GROUP_FID_EXTRA, -1));
    }
  ...
  public final static class Builder {
    ...
    //AA+
    public @NonNull Builder withFid(@Nullable Long fid) {
      this.fid = fid;
      return this;
    }

    //AA+
    public @NonNull Builder withChallenge(@Nullable String challenge) {
    ...
    public @NonNull Intent build() {
      ...
      if (fid <= 0)  {
        fid = SignalDatabase.threads().getFidForThreadId(threadId);
      }
      intent.putExtra(GROUP_FID_EXTRA, fid);//AA+
      if (challenge != null) intent.putExtra(CHALLENGE_EXTRA, challenge);//AA+
