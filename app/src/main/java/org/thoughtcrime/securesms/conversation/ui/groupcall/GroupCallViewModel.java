...
public class GroupCallViewModel extends ViewModel {
  ...
  public void peekGroupCall() {
    if (isGroupCallCapable(currentRecipient)) {
      ...
      ApplicationDependencies.getSignalCallManager().peekGroupCall(currentRecipient.getId());//.putExtra(WebRtcCallService.EXTRA_REMOTE_PEER, new RemotePeer(currentRecipient.getId(), -1));//AA+ -1
    }
  }
