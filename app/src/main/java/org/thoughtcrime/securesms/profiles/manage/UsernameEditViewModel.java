...
class UsernameEditViewModel extends ViewModel {
  ...
  private final MutableLiveData<Boolean>     isNicknameAvailable;//AA+
  ...
  private UsernameEditViewModel() {
    ...
    isNicknameAvailable = new MutableLiveData<>();
    repo.setupNicknameAvailabilityServiceReceiver(isNicknameAvailable::setValue);//AA+
  }

  void onUsernameUpdated(@NonNull String username, @Nullable Consumer<String> callback) {//AA+ callback
    ...
    if (username.equals(Recipient.self().getNickname())) {
      uiState.setValue(new State(ButtonState.SUBMIT_DISABLED, UsernameStatus.NONE));
      return;
    }
  ...
  void onUsernameSubmitted(@NonNull String username) {
    ...
    repo.setUsername(username, (result) -> {
      ThreadUtil.runOnMain(() -> {
        switch (result) {
          case SUCCESS:
            uiState.setValue(new State(ButtonState.SUBMIT_LOADING, UsernameStatus.NONE));//AA- SUBMIT_DISABLED
  ...
  //AA+
  void onNicknameLookupReturned(boolean isAvailable) {
    if (!isAvailable) {
      uiState.setValue(new State(ButtonState.SUBMIT_DISABLED, UsernameStatus.TAKEN));
      return;
    }
  ...
  //AA+
  public LiveData<Boolean> isNicknameAvailable() {
    return Transformations.distinctUntilChanged(isNicknameAvailable);
  }

  public void checkNicknameAvailability(String nickname)
  {
    repo.checkNicknameAvailability(nickname);
  }
