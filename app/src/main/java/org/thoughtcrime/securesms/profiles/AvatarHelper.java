...
public class AvatarHelper {
   ...
   * Deletes and avatar.
   */
  public static void delete(@NonNull Context context, @NonNull LocallyAddressable addressable) {
    getAvatarFile(context, addressable).delete();
  }
  ...
  //AA+
  public static boolean hasAvatar(@NonNull Context context, @NonNull LocallyAddressable addressable) {
    return getAvatarFile(context, addressable).exists();
  }
  ...
  //AA+
  public static @NonNull InputStream getAvatar(@NonNull Context context, @NonNull LocallyAddressable addressable) throws IOException {
    AttachmentSecret attachmentSecret = AttachmentSecretProvider.getInstance(context).getOrCreateAttachmentSecret();
    File             avatarFile       = getAvatarFile(context, addressable);
  ...
  //AA+
  public static void setAvatar(@NonNull Context context, @NonNull LocallyAddressable addressable, @Nullable InputStream inputStream)
          throws IOException
  {
    if (inputStream == null) {
      delete(context, addressable);
      return;
    }

    OutputStream outputStream = null;
    try {
      outputStream = getOutputStream(context, addressable, false);
      StreamUtil.copy(inputStream, outputStream);
    } finally {
      StreamUtil.close(outputStream);
    }
  ...
  //AA+
  public static @NonNull OutputStream getOutputStream(@NonNull Context context, @NonNull LocallyAddressable addressable, boolean isSyncAvatar) throws IOException {
    AttachmentSecret attachmentSecret = AttachmentSecretProvider.getInstance(context).getOrCreateAttachmentSecret();
    File             targetFile       = getAvatarFile(context, addressable, isSyncAvatar);
    return ModernEncryptingPartOutputStream.createFor(attachmentSecret, targetFile, true).second;
  }

  //AA+
  public static InputStream getInputStreamFor(@NonNull Context context, @NonNull LocallyAddressable addressable)
          throws IOException
  {
    return new FileInputStream(getAvatarFile(context, addressable));
  }
  ...
  //AA+
  public static boolean avatarExists(@NonNull Context context, @NonNull RecipientId recipientId) {
    File avatarFile = getAvatarFile(context, recipientId);
    return avatarFile.exists() && ModernEncryptingPartOutputStream.getPlaintextLength(getAvatarFile(context, recipientId).length()) > 0;
  }

  //AA+
  private static @NonNull File getAvatarFile(@NonNull Context context, @NonNull LocallyAddressable addressable) {
    File    profileAvatar       = getAvatarFile(context, addressable, false);
    boolean profileAvatarExists = profileAvatar.exists() && profileAvatar.length() > 0;
    File    syncAvatar          = getAvatarFile(context, addressable, true);
    boolean syncAvatarExists    = syncAvatar.exists() && syncAvatar.length() > 0;
  ...
  //AA+
  private static @NonNull File getAvatarFile(@NonNull Context context, @NonNull LocallyAddressable addressable, boolean isSyncAvatar) {
    File directory = context.getDir(AVATAR_DIRECTORY, Context.MODE_PRIVATE);
    return new File(directory, addressable.serialize() + (isSyncAvatar ? "-sync" : ""));
  }

  //AA+
  public static boolean avatarExists(@NonNull Context context, @NonNull LocallyAddressable addressable) {
    File avatarFile = getAvatarFile(context, addressable);
    return avatarFile.exists() && ModernEncryptingPartOutputStream.getPlaintextLength(getAvatarFile(context, addressable).length()) > 0;
  }
