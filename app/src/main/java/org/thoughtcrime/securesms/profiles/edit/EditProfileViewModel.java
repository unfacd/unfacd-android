...
class EditProfileViewModel extends ViewModel {
  ...
  private final MutableLiveData<String>           nickname            = new MutableLiveData<>();//AA+
  private final MutableLiveData<Boolean>          isNameEditable      = new MutableLiveData<>();//AA+
  ...
  private EditProfileViewModel(@NonNull EditProfileRepository repository, boolean hasInstanceState, @Nullable GroupId groupId) {
    ...
    repository.isNameEditable(isNameEditable::setValue);//AA+
    ...
    if (!hasInstanceState) {
      if (groupId != null) {
        ...
        repository.getCurrentNickname(name -> nickname.setValue(name.get()));//AA+
      }
  ...
  public LiveData<Boolean> isNameEditable() {
    return Transformations.distinctUntilChanged(isNameEditable);
  }
  //
