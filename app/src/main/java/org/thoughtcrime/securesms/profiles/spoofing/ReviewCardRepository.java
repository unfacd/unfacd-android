...
class ReviewCardRepository {
  ...
  int loadGroupsInCommonCount(@NonNull ReviewRecipient reviewRecipient) {
    return ReviewUtil.getGroupsInCommonCount(context, reviewRecipient.getRecipient());//.getId()); //AA-
  }
                                            ...
                                            @NonNull OnRecipientsLoadedListener onRecipientsLoadedListener)
  {
    SignalExecutors.BOUNDED.execute(() -> {
      ...
      List<RecipientId> recipientIds = SignalDatabase.recipients().getSimilarRecipientIds(resolved);
