...
public final class ReviewUtil {
  ...
  public static boolean isRecipientReviewSuggested(@NonNull RecipientId recipientId)
  {
    ...
    return SignalDatabase.recipients().getSimilarRecipientIds(recipient)
                                      .size() > 1;
  }
  ...
  //AA see alternative implementation below
//  @WorkerThread
...
//  }
  ...
  //AA+ alternative implementation to above
  @WorkerThread
  public static int getGroupsInCommonCount(@NonNull Context context,  @NonNull Recipient recipient) {
    return Stream.of(SignalDatabase.groups()
                             .getPushGroupsContainingMember(recipient.getUfsrvUid()))
            .filter(g -> g.getMembers().contains(Recipient.self().getId()))
            .map(GroupDatabase.GroupRecord::getRecipientId)
            .toList()
            .size();
