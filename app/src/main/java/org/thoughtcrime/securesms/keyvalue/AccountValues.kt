...
internal class AccountValues internal constructor(store: KeyValueStore) : SignalStoreValues(store) {
  ...
  companion object {
    ...
    //AA+
    @VisibleForTesting
    const val KEY_UFSRVUID = "account.ufsrvuid"
    @VisibleForTesting
    const val KEY_USERNAME= "account.username"
  }
  ...
  //AA+
  /** The local user's service username. */
  val ufsrvuid: String?
    get() = getString(KEY_UFSRVUID, null)

  fun setUfsrvUid(ufsrvuid: String) {
    putString(KEY_UFSRVUID, ufsrvuid)
  }

  /** The local user's [UfsrvUserId]. Will throw if not present. */
  fun requireUfsrvUid(): String {
    return requireNotNull(ufsrvuid){"UfsrvUid is undefined"}
  }

  val username: String?
    get() = getString(KEY_USERNAME, null)
  ...
  fun requireUsername(): String {
    return requireNotNull(username){"Username is undefined"}
  }
  //
