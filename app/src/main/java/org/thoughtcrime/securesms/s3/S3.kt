  ...
  fun <T> getAndVerifyObject(endpoint: String, clazz: Class<T>, md5: ByteArray? = getObjectMD5(endpoint)): ServiceResponse<T> {
    ...
    try {
      getObject(endpoint).use { response ->
        ...
        val md5Result = outputStream.sink().use { sink -> //AA calculate the hash of the content retrieved
...
//          return ServiceResponse.forExecutionError(Md5FailureException())//AA- apache currently doesn't return content hash in teh etag, so keep going
        }
  ...
  /**
   * Parses the MD5 from a response. etag: W/"7847faecb20130ba0c93f6519884eb71"
   * Etag: "203-63f1da3087b04" apache "(\").*([a-f0-9]{3}).*-([a-f0-9]{13}).*(\")"
   * "63f1da3087b04"
   */
  private fun getMD5FromResponse(response: Response): String? {
    val pattern: Pattern = Pattern.compile("(\").*([a-f0-9]{13})(\")")//(".*([a-f0-9]{32}).*") //AA-
    val header = response.header("Etag") ?: return null //AA+ Etag
    val matcher: Matcher = pattern.matcher(header)

    return if (matcher.find()) {
      matcher.group(2) //AA+ 2
    } else {
      null
    }
  }
