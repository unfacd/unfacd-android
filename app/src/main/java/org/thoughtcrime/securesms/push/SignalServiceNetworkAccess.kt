...
class SignalServiceNetworkAccess(context: Context) {
  ...
  val uncensoredConfiguration: SignalServiceConfiguration = SignalServiceConfiguration(
    arrayOf(SignalServiceUrl(BuildConfig.UFSRVAPI_URL, serviceTrustStore)),//AA+
    mapOf(
      0 to arrayOf(SignalCdnUrl(BuildConfig.UFSRVMEDIA_URL, serviceTrustStore)),//AA+
  ...
  fun isCensored(): Boolean {
    Log.w(TAG, ">>> isCensored() ALWAYS RETURNS FALSE")
    return false//AA+
