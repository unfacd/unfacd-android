...
public final class RemotePeer implements Remote, Parcelable
{
  ...
  private  long          groupFid;//AA+

  public RemotePeer(@NonNull RecipientId recipientId,  long groupFid) {
    this(recipientId, new CallId(-1L), groupFid);
  }

  public RemotePeer(@NonNull RecipientId recipientId, @NonNull CallId callId, long groupFid) {//AA+ fid
    this.recipientId        = recipientId;
    this.callState          = CallState.IDLE;
    this.callId             = callId;
    this.callStartTimestamp = 0;

    this.groupFid           = groupFid;//AA+
  }
  ...
  public Recipient getGroupRecipient() {
    return Recipient.resolved(RecipientUfsrvId.from(groupFid));
  }
  //
