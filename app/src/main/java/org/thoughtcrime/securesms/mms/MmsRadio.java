...
public class MmsRadio {
  ...
  private MmsRadio(Context context) {
    ...
    this.wakeLock            = powerManager.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "unfacd:mms");//AA+
  ...
  public synchronized void connect() throws MmsRadioException {
    ...
    if (status == APN_ALREADY_ACTIVE) {
      ...
      if (connectivityListener == null) {
        ...
        ContextCompat.registerReceiver(context, connectivityListener, filter, ContextCompat.RECEIVER_EXPORTED);
      }
