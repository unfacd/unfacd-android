...
public final class OutgoingGroupUpdateMessage extends OutgoingSecureMediaMessage {
  ...
  //AA for when groupContext is already serialised, eg coming out of the db
  public OutgoingGroupUpdateMessage(@NonNull Recipient recipient,
                                    ...
                                    @Nullable String ufsrvCommandWireBody)//AA+ not finalised yet
  {
    super(recipient,
          groupContext.getEncodedGroupContext(),
          avatar, sentTimeMillis,
  ...
  {
    this(recipient,
         new MessageGroupContext(group),
         getAttachments(avatar),
         sentTimeMillis,
         expireIn,
         viewOnce,
         quote,
         ...
         previews,
         mentions,
         Base64.encodeBytes(ufsrvCommandWire.toByteArray()));//AA+
  }
