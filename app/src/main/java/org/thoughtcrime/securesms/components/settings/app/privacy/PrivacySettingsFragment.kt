...
import com.unfacd.android.utils.UserPrefsUtils.UnsolicitedContactAction
...
class PrivacySettingsFragment : DSLSettingsFragment(R.string.preferences__privacy) {
  ...
  override fun onCreate(savedInstanceState: Bundle?) {
    ...
    ApplicationContext.getInstance().ufsrvcmdEvents.register(this);//AA+
  }
  ...
  private fun getConfiguration(state: PrivacySettingsState): DSLConfiguration {
    return configure {
      ...
      //AA+ not supported as a master switch
      /*  switchPref(
      ...
      //AA+
      clickPref(
        title = DSLSettingsText.from(R.string.preferences__unsolicited_contact_action_title),
        summary = DSLSettingsText.from(getUnsolicitedContactActionSummary(state.unsolicitedContactAction)),
        onClick = {
          onUnsolicitedContactActionClicked(state.unsolicitedContactAction)
        }
      )
      //
  ...
  //AA+
  @StringRes
  private fun getUnsolicitedContactActionSummary(unsolicitedContactAction: UnsolicitedContactAction): Int {
    return when (unsolicitedContactAction) {
      UnsolicitedContactAction.BLOCK -> R.string.preferences__unsolicited_contact_action_block
      UnsolicitedContactAction.ALLOW -> R.string.preferences__unsolicited_contact_action_allow
    }
  }

  private fun onUnsolicitedContactActionClicked(unsolicitedContactAction: UnsolicitedContactAction) {
    val value = arrayOf(unsolicitedContactAction)
    val items = itemsForUnsolicitedContactActions(requireContext())
    val modes: List<UnsolicitedContactAction> = ArrayList(items.keys)
    val modeStrings = items.values.toTypedArray()
    val selectedMode = modes.indexOf(value[0])

    MaterialAlertDialogBuilder(requireActivity()).apply {
      setTitle(R.string.preferences__unsolicited_contact_action_title)
      setCancelable(true)
      ...
      setPositiveButton(android.R.string.ok) { _: DialogInterface?, _: Int ->
        ...
        Log.i(
          TAG,
          String.format(
            "unsolicited contact action changed to %s. Syncing..",
            newSharingMode
          )
        )
        viewModel.syncUnsolicitedContactAction(value[0])
      }
      setNegativeButton(android.R.string.cancel, null)
      show()
    }
  ...
  private fun itemsForUnsolicitedContactActions(context: Context): Map<UnsolicitedContactAction, CharSequence> {
    val map: MutableMap<UnsolicitedContactAction, CharSequence> = LinkedHashMap()
    map[UnsolicitedContactAction.BLOCK] = titleAndDescription(
      context,
      context.getString(R.string.preferences__unsolicited_contact_action_block),
      context.getString(R.string.preferences__unsolicited_contact_action_block_summary)
    )
    map[UnsolicitedContactAction.ALLOW] = titleAndDescription(
      context,
      context.getString(R.string.preferences__unsolicited_contact_action_allow),
      context.getString(R.string.preferences__unsolicited_contact_action_allow_summary)
    )
    return map
  }
  //
  ...
  fun onEvent(event: AppEventUserPref) {
