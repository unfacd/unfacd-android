...
class PrivacySettingsViewModel(
  ...
  //AA+
  fun getUnsolicitedContactAction() : UserPrefsUtils.UnsolicitedContactAction {
    val action = TextSecurePreferences.getUnsolicitedContactAction(ApplicationDependencies.getApplication()).toInt()
    return UserPrefsUtils.UnsolicitedContactAction.values()[action]
  }

  fun syncUnsolicitedContactAction(unsolicitedContactAction: UserPrefsUtils.UnsolicitedContactAction) {
    UfsrvUserUtils.UfsrvSetMultivalueUserPreference(ApplicationDependencies.getApplication(), UfsrvUserUtils.myOwnRecipient(false), UserPrefs.UNSOLICITED_CONTACT, unsolicitedContactAction.ordinal);
  }

  fun mapUnsolicitedContactAction() : Boolean {
    when (getState().unsolicitedContactAction) {
      UserPrefsUtils.UnsolicitedContactAction.ALLOW -> true
      UserPrefsUtils.UnsolicitedContactAction.BLOCK -> false
    }
  ...
  private fun getState(): PrivacySettingsState {
    return PrivacySettingsState(
      ...
      unsolicitedContactAction = getUnsolicitedContactAction()//AA+
    )
  }
