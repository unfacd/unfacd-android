...
public class VoiceNotePlaybackService extends MediaBrowserServiceCompat {
      ...
      SignalExecutors.BOUNDED.execute(() -> {
        ...
        if (markedMessageInfo != null) {
          ApplicationDependencies.getJobManager().add(new SendViewedReceiptJob(markedMessageInfo.getThreadId(),
                                                                               ...
                                                                               markedMessageInfo.getSyncMessageId().getUfsrvMessageIdentifier()));//AA+
  ...
  private static class KeyClearedReceiver extends BroadcastReceiver {
    ...
    void register() {
      if (!registered) {
        ContextCompat.registerReceiver(context, this, KEY_CLEARED_FILTER, ContextCompat.RECEIVER_EXPORTED);
        registered = true;
      }
    }
  ...
  private static class BecomingNoisyReceiver extends BroadcastReceiver {
    ...
    void register() {
      if (!registered) {
        ContextCompat.registerReceiver(context, this, NOISY_INTENT_FILTER, ContextCompat.RECEIVER_EXPORTED);
        registered = true;
      }
    }
