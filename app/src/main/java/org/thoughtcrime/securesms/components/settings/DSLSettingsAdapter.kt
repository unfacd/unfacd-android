...
class DSLSettingsAdapter : MappingAdapter() {
  init {
    ...
    Text.register(this)
    Space.register(this)
    org.thoughtcrime.securesms.components.settings.models.Button.register(this)
    AsyncSwitch.register(this)
    registerFactory(CheckboxPreference::class.java, LayoutFactory(::CheckboxPreferenceViewHolder, R.layout.dsl_checkbox_preference_item))//AA+
  }
}
...
//AA+
class CheckboxPreferenceViewHolder(itemView: View) : PreferenceViewHolder<CheckboxPreference>(itemView) {

  private val checkboxWidget: MaterialCheckBox = itemView.findViewById(R.id.checkbox_widget)

  override fun bind(model: CheckboxPreference) {
    super.bind(model)
    checkboxWidget.isEnabled = model.isEnabled
    checkboxWidget.isChecked = model.isChecked
    itemView.setOnClickListener {
      model.onClick()
    }
  }
