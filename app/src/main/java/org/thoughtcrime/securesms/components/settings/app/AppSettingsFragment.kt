...
class AppSettingsFragment : DSLSettingsFragment(R.string.text_secure_normal__menu_settings) {
  ...
  private fun getConfiguration(state: AppSettingsState): DSLConfiguration {
    return configure {
      ...
      if (false) { //AA+ false disabled Payments option
      ...
      //AA+
      clickPref(
        title = DSLSettingsText.from(R.string.preferences__groups),
        icon = DSLSettingsIcon.from(R.drawable.ic_group_24),
        onClick = {
          findNavController().safeNavigate(R.id.action_appSettingsFragment_to_groupsSettingsFragment)
        }
      )
      //
      ...
      if (FeatureFlags.internalUser() || Recipient.self().requireUfsrvUid() == "3J140H9YY5H43K608000000000" || Recipient.self().requireUfsrvUid() == "3J140H9YY5H43KM08000000000") {
        dividerPref()
  ...
  private class BioPreferenceViewHolder(itemView: View) : PreferenceViewHolder<BioPreference>(itemView) {
    ...
    override fun bind(model: BioPreference) {
      ...
      summaryView.text = model.recipient.displayName//PhoneNumberFormatter.prettyPrint(model.recipient.requireE164()) //AA+
