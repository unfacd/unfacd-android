...
class ConversationSettingsRepository(
  ...
  fun getGroupsInCommon(recipientId: RecipientId, consumer: (List<Recipient>) -> Unit) {
    SignalExecutors.BOUNDED.execute {
      consumer(
        SignalDatabase
          .groups
          .getPushGroupsContainingMember(Recipient.resolved(recipientId).requireUfsrvUid())//AA+
          .asSequence()
          .filter { it.membersRecipientId.contains(Recipient.self().id) }//AA+ membersRecipientId
  ...
  fun getGroupMembership(recipientId: RecipientId, consumer: (List<RecipientId>) -> Unit) {
    SignalExecutors.BOUNDED.execute {
      ...
      val groupRecords = groupDatabase.getPushGroupsContainingMember(Recipient.resolved(recipientId).requireUfsrvUid())//AA+
  ...
  fun refreshRecipient(recipientId: RecipientId) {
    SignalExecutors.UNBOUNDED.execute {
      try {
        ContactDiscovery.refresh(LocallyAddressableUfsrvUid.from(Recipient.resolved(recipientId).requireServiceIdUfsrv()))//AA+
      } catch (e: IOException) {
      ...
      consumer(
        if (groupRecord.isV2Group) {
          ...
          val pendingMembers: List<RecipientId> = decryptedGroup.pendingMembersList
            .map(DecryptedPendingMember::getAci)
    ...
    SignalExecutors.BOUNDED.execute {
      ...
      consumer(
        if (true) {
          ...
          members.addAll(groupRecord.membersRecipientId)
          members.addAll(groupRecord.membersInvitedRecipientId)

          GroupCapacityResult(Recipient.self().id, members, FeatureFlags.groupLimits(), groupRecord.isAnnouncementGroup)
        } else {
          GroupCapacityResult(Recipient.self().id, groupRecord.membersRecipientId, FeatureFlags.groupLimits(), false)
        }
      )
    }
  }
...
//    SignalExecutors.BOUNDED.execute {//AA-
