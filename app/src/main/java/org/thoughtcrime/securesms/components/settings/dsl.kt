...
class DSLConfiguration {
  ...
  fun primaryButton(
    ...
    val preference = org.thoughtcrime.securesms.components.settings.models.Button.Model.Primary(text, null, isEnabled, onClick)
    children.add(preference)
  }
  ...
  fun secondaryButtonNoOutline(
    ...
    val preference = org.thoughtcrime.securesms.components.settings.models.Button.Model.SecondaryNoOutline(text, icon, isEnabled, onClick)
    children.add(preference)
  }
  ...
  fun checkboxPref(
    ...
    val preference = CheckboxPreference(title, summary, icon, isEnabled, isChecked, onClick)
    children.add(preference)
  }
...
class CheckboxPreference(
  ...
  val isChecked: Boolean,
  val onClick: () -> Unit
) : PreferenceModel<CheckboxPreference>(title = title, summary = summary, icon = icon, isEnabled = isEnabled) {
  override fun areContentsTheSame(newItem: CheckboxPreference): Boolean {
    return super.areContentsTheSame(newItem) && isChecked == newItem.isChecked
  }
}
//
