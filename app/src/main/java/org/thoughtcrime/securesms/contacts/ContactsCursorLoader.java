...
public class ContactsCursorLoader extends AbstractContactsCursorLoader {
  ...
  private Cursor getRecentConversationsCursor(boolean groupsOnly) {
    ...
    try (Cursor rawConversations = threadDatabase.getRecentConversationList(RECENT_CONVERSATION_MAX, flagSet(mode, DisplayMode.FLAG_INACTIVE_GROUPS), groupsOnly, !smsEnabled(mode))) {
  ...
  private Cursor getGroupsCursor() {
    ...
    try (GroupDatabase.Reader reader = SignalDatabase.groups().getGroupsFilteredByTitle(getFilter(), flagSet(mode, DisplayMode.FLAG_INACTIVE_GROUPS), !smsEnabled(mode))) {
