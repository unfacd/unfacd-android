...
public class ContactRepository {
  ...
  private @NonNull Cursor handleNoteToSelfQuery(@NonNull String query, boolean includeSelf, Cursor cursor) {
    ...
    if (includeSelf && noteToSelfTitle.toLowerCase().contains(query.toLowerCase())) {
      ...
      boolean   numberMatch = self.getUfsrvUidx().isPresent() && self.requireUfsrvUid().contains(query); //AA+
//      boolean   numberMatch = self.getE164().isPresent() && self.requireE164().contains(query);
      ...
      if (shouldAdd) {
        ...
        selfCursor.addRow(new Object[]{ self.getId().serialize(), noteToSelfTitle, null, self.getUfsrvUid(), self.getEmail().orElse(null), null, -1, RecipientDatabase.RegisteredState.REGISTERED.getId(), self.getAbout(), self.getAboutEmoji(), null, true, noteToSelfTitle, noteToSelfTitle });//AA+
