...
public abstract class GroupId {
  ...
  public static final String  ENCODED_SIGNAL_GROUP_V1_PREFIX  = "__uf_group__!";
  private static final String ENCODED_SIGNAL_GROUP_V2_PREFIX = "__uf_group__v2__!";
  private static final String ENCODED_MMS_GROUP_PREFIX       = "__uf_mms_group__!";
  ...
  public static final String ENCODED_GUARDIAN_GROUP_PREFIX    = "__uf_guardian_group__!";//AA+ local group
  public static final String ENCODED_UNDEFINED_GROUP_PREFIX    = "__uf_undefined_group__!";//AA+ local group
  public static final byte[] EMPTY_GROUPID  = new byte[V1_MMS_BYTE_LENGTH];//AA+
  public static final byte[] EMPTY_GROUPIDV2  = new byte[V2_BYTE_LENGTH];//AA+
  ...
  //AA++
  public static @NonNull GroupId.Guardian guradian(byte[] guardianGroupIdBytes) {
    return new GroupId.Guardian(guardianGroupIdBytes);
  }

  //AA+
  public static @NonNull byte[] requireEmptyId() {
    return EMPTY_GROUPIDV2;
  }

  //AA+
  public static boolean isGroupIdAllocated (byte[] groupId) {
    return !Arrays.equals(groupId, EMPTY_GROUPID);
  }

  //AA+
  public static boolean isGroupIdV2Allocated (byte[] groupId) {
    return !Arrays.equals(groupId, EMPTY_GROUPIDV2);
  }

  public static GroupId.Push push(byte[] bytes) throws BadGroupIdException {
    return GroupId.v2orThrow(bytes);//bytes.length == V2_BYTE_LENGTH ? v2(bytes) : v1(bytes);//AA-
  }
  ...
  public static @NonNull GroupId parse(@NonNull String encodedGroupId) throws BadGroupIdException {
    try {
      ...
      else if (encodedGroupId.startsWith(ENCODED_GUARDIAN_GROUP_PREFIX))  return guradian(bytes);//AA+
  ...
  public static boolean isEncodedGroup(@NonNull String groupId) {
    return groupId.startsWith(ENCODED_SIGNAL_GROUP_V2_PREFIX) ||
            ...
            groupId.startsWith(ENCODED_GUARDIAN_GROUP_PREFIX);//AA+
  }
  ...
  public abstract boolean isGuardian();//AA++
  ...
  //AA++
  public GroupId.Guardian requireGuardian() {
    if (this instanceof GroupId.Guardian) return (GroupId.Guardian) this;
    throw new AssertionError();
  }
  ...
  //AA++
  public static final class Guardian extends GroupId {

    private Guardian(@NonNull byte[] bytes) {
      super(ENCODED_GUARDIAN_GROUP_PREFIX, bytes);
    }
