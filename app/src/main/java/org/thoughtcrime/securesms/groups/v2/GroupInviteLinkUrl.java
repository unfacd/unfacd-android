...
public final class GroupInviteLinkUrl {
  ...
  private static final String GROUP_URL_HOST   = "api.unfacd.io";//AA+
  ...
  public static GroupInviteLinkUrl forGroup(@NonNull GroupMasterKey groupMasterKey,
                                            ...
                                            @NonNull Long fid)//AA+
  {
    return new GroupInviteLinkUrl(groupMasterKey, GroupLinkPassword.fromBytes(group.getInviteLinkPassword().toByteArray()), fid);
  }
    ...
    try {
      ...
      switch (groupInviteLink.getContentsCase()) {
        case V1CONTENTS: {
          ...
          return new GroupInviteLinkUrl(groupMasterKey, password, Long.valueOf(-1));//A+
        }
        case V10CONTENTS: {//AA+
          GroupInviteLink.GroupInviteLinkContentsV10 groupInviteLinkContentsV10   = groupInviteLink.getV10Contents();
          GroupMasterKey                            groupMasterKey                = new GroupMasterKey(groupInviteLinkContentsV10.getGroupMasterKey().toByteArray());
          GroupLinkPassword                         password                      = GroupLinkPassword.fromBytes(groupInviteLinkContentsV10.getInviteLinkPassword().toByteArray());
          Long                                      fid                           = groupInviteLinkContentsV10.getFid();

          return new GroupInviteLinkUrl(groupMasterKey, password, Long.valueOf(fid));//A+
        }
  ...
  private static URI getGroupUrl(@NonNull String urlString) {
    try {
      ...
      if (!"https".equalsIgnoreCase(url.getScheme()) &&
          !"unfcd".equalsIgnoreCase(url.getScheme()))//AA+
      {
        return null;
      }
  ...
  public GroupInviteLinkUrl(@NonNull GroupMasterKey groupMasterKey, @NonNull GroupLinkPassword password, @NonNull Long fid) {//AA+ public + fid
    this.groupMasterKey = groupMasterKey;
    this.password       = password;
    this.fid            = fid;//AA+
    this.url            = createUrl(groupMasterKey, password, fid);

  }

  protected static @NonNull String createUrl(@NonNull GroupMasterKey groupMasterKey, @NonNull GroupLinkPassword password, @NonNull Long fid) {//AA+
    GroupInviteLink groupInviteLink = GroupInviteLink.newBuilder()
                                                     .setV10Contents(GroupInviteLink.GroupInviteLinkContentsV10.newBuilder()
