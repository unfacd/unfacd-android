...
final class GiphyMp4PagedDataSource implements PagedDataSource<String, GiphyImage> {
  ...
  GiphyMp4PagedDataSource(@Nullable String searchQuery) {
    ...
    this.client       = ApplicationDependencies.getOkHttpClient().newBuilder().proxySelector(new ContentProxySelector()).proxy(Proxy.NO_PROXY).build();//AA+ .proxy(Proxy.NO_PROXY)
  }
