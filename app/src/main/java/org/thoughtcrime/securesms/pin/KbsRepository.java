...
import com.unfacd.android.KbsEnclave;
...
public final class KbsRepository {
  ...
  public static synchronized @Nullable KbsPinData restoreMasterKey(@Nullable String pin,
                                                                   ...
                                                                   Optional<SignalServiceAccountManager> accountManager)//AA+
          throws IOException, KeyBackupSystemWrongPinException, KeyBackupSystemNoDataException
  {
    ...
    return restoreMasterKeyFromEnclave(enclave, pin, basicStorageCredentials, tokenResponse, accountManager);
  }
  ...
  private static @NonNull KbsPinData restoreMasterKeyFromEnclave(@NonNull KbsEnclave enclave,
          ...
          throws IOException, KeyBackupSystemWrongPinException, KeyBackupSystemNoDataException
  {
    //AA+ custom injected account manager, as credentials are not yet available
    KeyBackupService                keyBackupService;
    if (accountManager.isPresent()) {
      keyBackupService = accountManager.get()
                                       .getKeyBackupService(IasKeyStore.getIasKeyStore(ApplicationDependencies.getApplication()),//AA+ true
                                                            enclave.getEnclaveName(),
                                                            Hex.fromStringOrThrow(enclave.getServiceId()),
                                                            enclave.getMrEnclave(),
                                                            10);
    } else {
      keyBackupService = ApplicationDependencies.getKeyBackupService(enclave);
    }
   //
