...
class EmojiKeyboardPageFragment : Fragment(), EmojiEventListener, EmojiPageViewGridAdapter.VariationSelectorListener, KeyboardPageSelected {
  ...
  //AA+ was deleted in https://github.com/signalapp/Signal-Android/commit/664d6475d950612d754bd5767ec53ede86f63dc5
 /* override fun onAttach(context: Context) {
    super.onAttach(context)

    //+ to enable working with DialogFragment
    val prev = requireActivity().supportFragmentManager.findFragmentByTag("intro_dialog")
    callback = if (prev != null) {
      prev as Callback
    } else
    //
      context as Callback

    if (prev != null) eventListener = prev as EmojiEventListener//AA+ to enable working with DialogFragment
  }*/
  ...
  override fun onActivityCreated(savedInstanceState: Bundle?) {
    ...
    if (!::eventListener.isInitialized) //AA+ conditional to enable working with DialogFragment
