...
public abstract class BaseEnterSmsCodeFragment<ViewModel extends BaseRegistrationViewModel> extends LoggingFragment implements SignalStrengthPhoneStateListener.Callback {
  ...
  private TextView                subheader;//AA+

  protected final LifecycleDisposable disposables = new LifecycleDisposable();
  protected Runnable runnableEmailVerificationListener;//AA+
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    subheader            = view.findViewById(R.id.verify_subheader);//AA+
    ...
    callMeCountDown.setListener((v, remaining) -> {
      if (remaining <= 30) {
        ...
        viewModel.cancelEmailVerifiedListener(runnableEmailVerificationListener);//AA+
      }
    });
    ...
    runnableEmailVerificationListener = getViewModel().launchEmailVerifiedListener();//AA+
  ...
  private void handlePhoneCallRequest() {
   /* showConfirmNumberDialogIfTranslated(requireContext(),
                                        ...
                                        this::onWrongNumber);*///AA-
    if (runnableEmailVerificationListener != null) {
      runnableEmailVerificationListener = getViewModel().launchEmailVerifiedListener();//AA+
    }
    getViewModel().onStartEnterCode();//AA+
  }
  ...
  public void onResume() {
    ...
    //AA+
//    header.setText(requireContext().getString(R.string.RegistrationActivity_enter_the_code_we_sent_to_s, viewModel.getNumber().getFullFormattedNumber()));//AA-
    header.setText(requireContext().getString(R.string.RegistrationActivity_your_email_is_being_verified));
    subheader.setText(getString(R.string.RegistrationActivity_check_your_email_for_verification_link, viewModel.getEmail().getEmail()));
//    keyboard.setVisibility(View.INVISIBLE); //AA keep visible, otherwise fragment won't transition to rego-locked flow
    //
