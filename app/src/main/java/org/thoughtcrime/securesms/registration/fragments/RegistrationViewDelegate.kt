...
object RegistrationViewDelegate {
  ...
  @JvmStatic
  fun showConfirmEmailDialogIfTranslated(
    context: Context,
    @StringRes firstMessageLine: Int,
    email: String,
    onConfirmed: Runnable,
    onEditEmail: Runnable
  ) {
    val translationDetection = TranslationDetection(context)

    if (translationDetection.textExistsInUsersLanguage(
        firstMessageLine,
        R.string.RegistrationActivity_is_your_email_above_correct,
        R.string.RegistrationActivity_edit_email
      )
    ) {
      val message: CharSequence = SpannableStringBuilder()
        .append(context.getString(firstMessageLine))
        .append("\n\n")
        .append(SpanUtil.bold(email))
        .append("\n\n")
        .append(context.getString(R.string.RegistrationActivity_is_your_email_above_correct))

      MaterialAlertDialogBuilder(context)
        .setMessage(message)
        .setPositiveButton(android.R.string.ok) { _, _ -> onConfirmed.run() }
        .setNegativeButton(R.string.RegistrationActivity_edit_email) { _, _ -> onEditEmail.run() }
        .show()
    } else {
      onConfirmed.run()
    }
