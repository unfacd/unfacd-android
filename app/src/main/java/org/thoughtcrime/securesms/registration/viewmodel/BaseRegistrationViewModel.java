...
public abstract class BaseRegistrationViewModel extends ViewModel {
  ...
  private static final String STATE_EMAIL                            = "EMAIL";//AA+
  private static final String STATE_REGO_COOKIE                      = "REGO_COOKIE";
  ...
  //AA+
  private VerifyAccountRepository.EmailVerifiedListener verificationLister;
  Runnable runnableVerificationListener;
  //
                                   ...
                                   @NonNull String password)
  {
    ...
    //AA+
    setInitialDefaultValue(STATE_EMAIL, EmailViewState.INITIAL);
    setInitialDefaultValue(STATE_REGO_COOKIE, "");
    //AA+
  }
  ...
  //AA+
  public @NonNull EmailViewState getEmail() {
    //noinspection ConstantConditions
    return savedState.get(STATE_EMAIL);
  }

  public void setEmail(String email) {
    EmailViewState emailViewState = getEmail().toBuilder().email(email).build();
    setEmailViewState(emailViewState);
  }

  protected void setEmailViewState(EmailViewState emailViewState) {
    if (!emailViewState.equals(getEmail())) {
      savedState.set(STATE_EMAIL, emailViewState);
    }
  }

  public @NonNull LiveData<EmailViewState> getLiveEmail() {
    return savedState.getLiveData(STATE_EMAIL);
  }

  public void setRegoCookie(String regoCookie)
  {
    savedState.set(STATE_REGO_COOKIE, regoCookie);
  }

  public @Nullable String getRegoCookie() {
    return savedState.get(STATE_REGO_COOKIE);
  }
  //
  ...
  //AA+
  public Runnable launchEmailVerifiedListener() {
    verificationLister = new VerifyAccountRepository.EmailVerifiedListener(new Handler(Looper.getMainLooper()));
    Runnable runnable = verificationLister.prepare();
    verificationLister.launch(runnable);
    return runnable;
  }

  public void cancelEmailVerifiedListener(Runnable runnable) {
    verificationLister.removeEmailVerifiedListener(runnable);
  }
  //
  ...
  public Single<RequestVerificationCodeResponseProcessor> requestVerificationCode(@NonNull Mode mode) {
    ...
    if (mode == Mode.PHONE_CALL) {
      onCallRequested();
    } else if (!getRequestLimiter().canRequest(mode, getEmail().getEmail(), System.currentTimeMillis())) {//AA+ email
    ...
    return verifyAccountRepository.requestVerificationCode(getNumber().getE164Number(),
                                  ...
                                  .doOnSuccess(processor -> {
                                    if (processor.hasResult()) {
                                      ...
                                      getRequestLimiter().onSuccessfulRequest(mode, getEmail().getEmail(), System.currentTimeMillis());
                                    } else {
...
//                                    .map(r -> new VerifyAccountResponseWithSuccessfulKbs(processor.getResponse(), r.getResult().orElse(null)));//AA+
