...
public final class RegistrationNavigationActivity extends AppCompatActivity {
  ...
  private void initializeChallengeListener() {
    ...
    registerReceiver(smsRetrieverReceiver, new IntentFilter(SmsRetriever.SMS_RETRIEVED_ACTION), ContextCompat.RECEIVER_EXPORTED);*/

    IntentFilter filter = new IntentFilter();
    filter.addAction(RegistrationStatusVerifierJob.REGO_STATUS_VERIFIED_ACTION);

    smsRetrieverReceiver = new SmsRetrieverReceiver();
    ContextCompat.registerReceiver(getApplicationContext(), smsRetrieverReceiver, filter, ContextCompat.RECEIVER_EXPORTED);
  }

  private void shutdownChallengeListener() {
    if (smsRetrieverReceiver != null) {//AA+ rearranged as it was crashing on pixel 7 api 36
      try {
        unregisterReceiver(smsRetrieverReceiver);
      } catch (IllegalArgumentException x) {
        Log.w(TAG, "SmsRetrieverReceiver was already unregistered.");
      }
    }
  ...
  private class SmsRetrieverReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      if (SmsRetriever.SMS_RETRIEVED_ACTION.equals(intent.getAction())) {
        ...
        Log.w(TAG, "SmsRetrieverReceiver received EMAIL ACCOUNT VERIFIED");
