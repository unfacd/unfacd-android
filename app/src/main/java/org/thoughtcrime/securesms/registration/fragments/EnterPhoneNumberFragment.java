...
import org.thoughtcrime.securesms.registration.util.RegistrationEmailInputController;
...
import static org.thoughtcrime.securesms.registration.fragments.RegistrationViewDelegate.showConfirmEmailDialogIfTranslated;
...
public final class EnterPhoneNumberFragment extends LoggingFragment implements RegistrationNumberInputController.Callbacks, RegistrationEmailInputController.CallbacksForUfRegistration {//AA+ RegistrationEmailInputController
  ...
  private LabeledEditText        email;//AA+
  ...
  public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
    ...
    RegistrationEmailInputController controllerEmail = new RegistrationEmailInputController(requireContext(), email, true, this);
    //
    ...
    viewModel.getLiveEmail().observe(getViewLifecycleOwner(), controllerEmail::updateEmail);//AA+
  ...
  private void handleRegister(@NonNull Context context) {
    if (TextUtils.isEmpty(email.getText())) {
      Toast.makeText(context, getString(R.string.RegistrationActivity_you_must_specify_your_email_address), Toast.LENGTH_LONG).show();
      return;
    }

    final EmailViewState emailViewState = viewModel.getEmail();
    if (!emailViewState.isValid()) {
      Dialogs.showAlertDialog(context,
                              getString(R.string.RegistrationActivity_invalid_email),
                              String.format(getString(R.string.RegistrationActivity_the_email_you_specified_s_is_invalid), emailViewState.getEmail()));
      return;
    }

    final NumberViewState numberViewState     = viewModel.getNumber();
    final String          e164number = numberViewState.getE164Number();

    if (!TextUtils.isEmpty(number.getText())) {
      if (!numberViewState.isValid()) {
        Dialogs.showAlertDialog(context,
    ...
    if (fcmStatus == PlayServicesUtil.PlayServicesStatus.SUCCESS) {
      confirmEmailPrompt(context, emailViewState.getEmail(), () -> handleRequestVerification(context, true));
    } else if (fcmStatus == PlayServicesUtil.PlayServicesStatus.MISSING) {
      confirmNumberPrompt(context, emailViewState.getEmail(), () -> handlePromptForNoPlayServices(context));
  ...
  private void handleRegisterOrig(@NonNull Context context) {
  ...
  private void handleRequestVerification(@NonNull Context context, boolean fcmSupported) {
    ...
    if (fcmSupported) {
      ...
      task.addOnSuccessListener(none -> {
        ...
        requestVerificationCode(Mode.WEB_LINK);//AA+ //Mode.SMS_WITH_LISTENER);
      });
  ...
  //AA+
  @Override
  public void onEmailFocused() {
    scrollView.postDelayed(() -> scrollView.smoothScrollTo(0, register.getBottom()), 250);
  }

  @Override
  public void onEmailInputNext(@NonNull View view) {
    // Intentionally left blank
  }

  @Override
  public void onEmailInputDone(@NonNull View view) {
    ViewUtil.hideKeyboard(requireContext(), view);
    handleRegister(requireContext());
  }
  ...
  //AA+
  protected final void confirmEmailPrompt(@NonNull Context context,
                                           @NonNull String registrationEmail,
                                           @NonNull Runnable onConfirmed)
  {
    showConfirmEmailDialogIfTranslated(context,
                                        R.string.RegistrationActivity_a_verification_code_will_be_sent_to,
                                       registrationEmail,
                                        () -> {
                                          ViewUtil.hideKeyboard(context, email.getInput());
                                          onConfirmed.run();
                                        },
                                        () -> email.focusAndMoveCursorToEndAndOpenKeyboard());
  }
