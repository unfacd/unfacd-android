...
public class ContentProxySelector extends ProxySelector {
  ...
  private final List<Proxy> CONTENT = new ArrayList<Proxy>(1) {{
    add(new Proxy(Proxy.Type.HTTP, InetSocketAddress.createUnresolved(BuildConfig.GIPHY_PROXY_HOST,
                                                                      BuildConfig.CONTENT_PROXY_PORT)));
  }};

  @Override
  public List<Proxy> select(URI uri) {
    return new LinkedList<Proxy>(){{ //AA+ simply route back the same domain for a direct connect.. no proxy
      add(new Proxy(Proxy.Type.HTTP, new InetSocketAddress(uri.getHost(),
                                                           BuildConfig.CONTENT_PROXY_PORT)));
    }};
