...
final class RecipientDialogViewModel extends ViewModel {
  ...
  //AA+
  void onSecureCallClicked(@NonNull FragmentActivity  activity, GroupId groupId) {//AA+ groupId
    recipientDialogRepository.getRecipient(recipient -> CommunicationActions.startVoiceCall(activity, recipient, SignalDatabase.groups().getGroupFid(groupId).get()));//AA+
  }
  ...
  void onSecureVideoCallClicked(@NonNull FragmentActivity activity, GroupId groupId) {//AA+ groupId
    recipientDialogRepository.getRecipient(recipient -> CommunicationActions.startVideoCall(activity, recipient, SignalDatabase.groups().getGroupFid(groupId).get()));//AA+
  }

  void onBlockClicked(@NonNull FragmentActivity activity) {
    recipientDialogRepository.getRecipient(recipient -> BlockUnblockDialog.showBlockFor(activity, activity.getLifecycle(), recipient, () -> UfsrvUserUtils.UfsrvShareBlocking(context, recipient, true)));//RecipientUtil.blockNonGroup(context, recipient)));
  }

  void onUnblockClicked(@NonNull FragmentActivity activity) {
    recipientDialogRepository.getRecipient(recipient -> BlockUnblockDialog.showUnblockFor(activity, activity.getLifecycle(), recipient, () -> UfsrvUserUtils.UfsrvShareBlocking(context, recipient, false)));//RecipientUtil.unblock(context, recipient)));
  }

  //AA+
  void onEnableReadReceiptsClicked(@NonNull FragmentActivity activity) {
    new MaterialAlertDialogBuilder(activity)
                   ...
                   .setPositiveButton(R.string.preferences_dialog_share_profile_key,
                                      (dialog, which) -> {
                                        recipientDialogRepository.setReadReceiptsSharing(true, result -> {
...
//                                                                                             Toast.makeText(activity, R.string.ManageGroupActivity_failed_to_update_the_group, Toast.LENGTH_SHORT).show();
  ...
  void onDisableReadReceiptsClicked(@NonNull FragmentActivity activity) {
    new AlertDialog.Builder(activity)
                   ...
                   .setCancelable(true)
                   .setNegativeButton(android.R.string.cancel, (dialog, which) -> {})
                   .setPositiveButton(R.string.preferences_dialog_unshare_profile_key,
                                      (dialog, which) -> {
                                        recipientDialogRepository.setReadReceiptsSharing(false, result -> {
                                                                                          adminActionBusy.setValue(false);
                                                                                          if (!result) {
//                                                                                            Toast.makeText(activity, R.string.ManageGroupActivity_failed_to_update_the_group, Toast.LENGTH_SHORT).show();
                                                                                          }
  ...
  void onEnableProfileSharingClicked(@NonNull FragmentActivity activity) {
    new MaterialAlertDialogBuilder(activity)
            .setTitle(R.string.preferences_groups__share_profile_key)
            .setMessage(R.string.preferences_groups__share_profile_key_summary)
            .setCancelable(true)
            .setNegativeButton(android.R.string.cancel, (dialog, which) -> {})
            .setPositiveButton(R.string.preferences_dialog_share_profile_key,
                               (dialog, which) -> {
                                 recipientDialogRepository.setProfileSharing(true, result -> {
                                                                                    adminActionBusy.setValue(false);
                                                                                    if (!result) {
//                                                                                             Toast.makeText(activity, R.string.ManageGroupActivity_failed_to_update_the_group, Toast.LENGTH_SHORT).show();
                                                                                    }
  ...
  void onDisableProfileSharingClicked(@NonNull FragmentActivity activity) {
    new AlertDialog.Builder(activity)
            ...
            .setPositiveButton(R.string.preferences_dialog_unshare_profile_key,
                               (dialog, which) -> {
                                 recipientDialogRepository.setProfileSharing(false, result -> {
                                                                                    adminActionBusy.setValue(false);
                                                                                    if (!result) {
//                                                                                            Toast.makeText(activity, R.string.ManageGroupActivity_failed_to_update_the_group, Toast.LENGTH_SHORT).show();
                                                                                    }
  ...
  void onEnablePresenceSharingClicked(@NonNull FragmentActivity activity) {
    new MaterialAlertDialogBuilder(activity)
            .setTitle(R.string.preferences_share_presence)
            .setMessage(R.string.preferences_share_presence_summary)
            .setCancelable(true)
            .setNegativeButton(android.R.string.cancel, (dialog, which) -> {})
            .setPositiveButton(R.string.preferences_dialog_share_profile_key,
                               (dialog, which) -> {
                                 recipientDialogRepository.setPresenceSharing(true, result -> {
                                                                                    adminActionBusy.setValue(false);
                                                                                    if (!result) {
//                                                                                             Toast.makeText(activity, R.string.ManageGroupActivity_failed_to_update_the_group, Toast.LENGTH_SHORT).show();
                                                                                    }
  ...
  void onDisablePresenceSharingClicked(@NonNull FragmentActivity activity) {
    new AlertDialog.Builder(activity)
            ...
            .setPositiveButton(R.string.preferences_dialog_unshare_profile_key,
                               (dialog, which) -> {
                                 recipientDialogRepository.setPresenceSharing(false, result -> {
                                                                                    adminActionBusy.setValue(false);
                                                                                    if (!result) {
//                                                                                            Toast.makeText(activity, R.string.ManageGroupActivity_failed_to_update_the_group, Toast.LENGTH_SHORT).show();
                                                                                    }
