...
final class RecipientDialogRepository {
  ...
  void refreshRecipient() {
    SignalExecutors.UNBOUNDED.execute(() -> {
      try {
        ContactDiscovery.refresh(LocallyAddressableUfsrvUid.from(Recipient.resolved(recipientId).requireUfsrvUid()));//AA+
      } catch (IOException e) {
  ...
  void getGroupMembership(@NonNull Consumer<List<RecipientId>> onComplete) {
    SimpleTask.run(SignalExecutors.UNBOUNDED,
                   () -> {
                     ...
                     List<GroupDatabase.GroupRecord> groupRecords    = groupDatabase.getPushGroupsContainingMember(Recipient.resolved(recipientId).getUfsrvUid());//AA+
  ...
  //AA+
  void setReadReceiptsSharing(boolean isSharing, @NonNull Consumer<Boolean> onComplete, @NonNull GroupChangeErrorCallback error) {
    SimpleTask.run(SignalExecutors.UNBOUNDED,
                   () -> {
                     try {//this is just a mock
                       UfsrvUserUtils.UfsrvShareReadReceipt(context, Recipient.resolved(recipientId), isSharing);
                       return true;
                     } catch (Exception e) {
                       Log.w(TAG, e);
                       error.onError(GroupChangeFailureReason.fromException(e));
  ...
  void setProfileSharing(boolean isSharing, @NonNull Consumer<Boolean> onComplete, @NonNull GroupChangeErrorCallback error) {
    SimpleTask.run(SignalExecutors.UNBOUNDED,
                   () -> {
                     try {
                       UfsrvUserUtils.UfsrvShareProfileWithRecipient(context, Recipient.resolved(recipientId), isSharing);
                       return true;
                     } catch (Exception e) {
                       Log.w(TAG, e);
                       error.onError(GroupChangeFailureReason.fromException(e));
  ...
  void setPresenceSharing(boolean isSharing, @NonNull Consumer<Boolean> onComplete, @NonNull GroupChangeErrorCallback error) {
    SimpleTask.run(SignalExecutors.UNBOUNDED,
                   () -> {
                     try {
                       UfsrvUserUtils.UfsrvSharePresence(context, Recipient.resolved(recipientId), isSharing);
                       return true;
                     } catch (Exception e) {
                       Log.w(TAG, e);
                       error.onError(GroupChangeFailureReason.fromException(e));
  ...
  //AA not in use. See specialised block dialog BlockUnlockDialog
  void setRecipientBlocked(boolean isBlocked, @NonNull Consumer<Boolean> onComplete, @NonNull GroupChangeErrorCallback error) {
    SimpleTask.run(SignalExecutors.UNBOUNDED,
                   () -> {
                     try {
                       UfsrvUserUtils.UfsrvShareBlocking(context, Recipient.resolved(recipientId), isBlocked);
                       return true;
                     } catch (Exception e) {
                       Log.w(TAG, e);
                       error.onError(GroupChangeFailureReason.fromException(e));
