...
import static org.thoughtcrime.securesms.database.GroupDatabase.GROUP_MODE_DEVICELOCAL;
...
public class RecipientUtil {
          ...
          throws IOException
  {
    ...
    if (!recipient.hasUfsrvUid()) {
//      throw new AssertionError(recipient.getId() + " - No UfsrvUid is defined!");
      Log.e(TAG, recipient.getId() + " is missing a ufsrvuid...");
      return new SignalServiceAddress(recipient.requireServiceId(), Optional.of(UfsrvUid.UndefinedUfsrvUid));
    }

//    return new SignalServiceAddress(recipient.requireServiceId(), Optional.ofNullable(recipient.getUfsrvUid()));
    return new SignalServiceAddress(recipient.getServiceId().orElse(null), Optional.ofNullable(recipient.getUfsrvUid()));
...
//    }
//
//    if (FeatureFlags.cds() && !recipient.getAci().isPresent()) {
...
//    }
//
//    return new SignalServiceAddress(Optional.ofNullable(recipient.getAci().orElse(null)), Optional.ofNullable(recipient.resolve().getE164().orElse(null)));
//
  }
          ...
          throws IOException
  {
    ...
    return Stream.of(recipients)
                 ...
                 .map(r -> new SignalServiceAddress(r.getServiceId().orElse(null), r.requireUfsrvUid()))//AA+ ufsrvuid
                 .toList();
  }
          ...
          throws IOException
  {
    ...
    if (recipientsWithoutUuids.size() > 0) {
      ContactDiscovery.refresh(recipientsWithoutUuids);

      if (recipients.stream().map(Recipient::resolve).anyMatch(Recipient::isUnregistered)) {
//        throw new NotFoundException("1 or more recipients are not registered!");//AA-
        Log.d(TAG, "1 or more recipients are not registered!");
      }
  ...
  public static List<Recipient> getEligibleForSending(@NonNull List<Recipient> recipients) {
    return recipients;//AA+ below is valid too, but we let the backend decide for now
    /*return Stream.of(recipients)
  ...
  public static boolean isMessageRequestAccepted(@NonNull Context context, long threadId) {
    ...
    GroupDatabase  groupDatabase   = SignalDatabase.groups();//AA+
    int groupMode = groupDatabase.getGroupMode(threadRecipient.requireGroupId());

    if (threadRecipient == null) {
      return true;
    }

    return isMessageRequestAccepted(context, threadId, threadRecipient, groupMode);//AA+
  }
  ...
  //AA+
  private static boolean messageRequestAcceptedFromGroupMode(int groupMode)
  {
    return groupMode != GROUP_MODE_DEVICELOCAL &&
            groupMode != GROUP_MODE_INVITATION &&
            groupMode != GROUP_MODE_LINKJOIN_REQUESTING;/*== GroupDatabase.GROUP_MODE_JOIN_ACCEPTED        ||
            groupMode == GroupDatabase.GROUP_MODE_LINKJOIN_ACCEPTED   ||
            groupMode == GroupDatabase.GROUP_MODE_LEAVE_ACCEPTED      ||
            groupMode == GroupDatabase.GROUP_MODE_GEOBASED_JOIN       ||
            groupMode == GroupDatabase.GROUP_MODE_LEAVE_GEO_BASED     ||
            groupMode == GroupDatabase.GROUP_MODE_INVITATION_REJECTED ||
            groupMode == GroupDatabase.GROUP_MODE_BLOCKED             ||
            groupMode == GroupDatabase.GROUP_MODE_GUARDIAN;*/
  }
  ...
  public static boolean shouldHaveProfileKey(@NonNull Context context, @NonNull Recipient recipient) {
    ...
    if (recipient.isProfileSharing()) {
      ...
      return groupDatabase.getPushGroupsContainingMember(recipient.getUfsrvUid())//AA+ ufsrvuid
                          .stream()
  ...
  public static boolean setAndSendUniversalExpireTimerIfNecessary(@NonNull Context context, @NonNull Recipient recipient, long threadId) {
    ...
    if (threadId == -1 || !SignalDatabase.mmsSms().hasMeaningfulMessage(threadId)) {
      ...
      OutgoingExpirationUpdateMessage outgoingMessage = new OutgoingExpirationUpdateMessage(recipient, System.currentTimeMillis(), defaultTimer * 1000L, "");//AA+
      MessageSender.send(context, outgoingMessage, SignalDatabase.threads().getOrCreateThreadIdFor(recipient), false, null, null, UfsrvCommand.TransportType.UNDEFINED, false);//AA+
      return true;
    }
    return false;
  }
  ...
  @WorkerThread
  public static boolean isMessageRequestAccepted(@NonNull Context context, @Nullable Long threadId, @NonNull Recipient threadRecipient, int groupMode) {//AA+ groupmode
    return messageRequestAcceptedFromGroupMode(groupMode)         ||//AA+
  ...
  private static boolean isCallRequestAccepted(@NonNull Context context, @Nullable Long threadId, @NonNull Recipient threadRecipient) {
    AtomicBoolean isTrueSettable = new AtomicBoolean(false);
    SignalDatabase.groups().getGroupIdFromThreadId(threadId).ifPresent(groupId -> isTrueSettable.set(SignalDatabase.groups().isPairedGroupAndAmOnIt(groupId)));
    return isTrueSettable.get();//AA+
  ...
  private static boolean isProfileSharedViaGroup(@NonNull Context context, @NonNull Recipient recipient) {
    return Stream.of(SignalDatabase.groups().getPushGroupsContainingMember(recipient.getUfsrvUid()))//AA+ ufsrvuid
