...
public class RecipientId implements Parcelable, Comparable<RecipientId>, DatabaseId {
  ...
  private static final long UFSRV_ID   = 0;//AA+

  public static final RecipientId UNKNOWN = RecipientId.from(UNKNOWN_ID);
  public static final RecipientId UFSRV = RecipientId.from(UFSRV_ID);//AA+
  ...
  @AnyThread
  @SuppressLint("WrongThread")
  public static @NonNull RecipientId from(@NonNull UfsrvUid ufsrvUid) {
    RecipientId recipientId = RecipientIdCache.INSTANCE.get(ufsrvUid);

    if (recipientId == null) {
      Recipient recipient = Recipient.resolvedFromUfsrvUid(ufsrvUid.toString());
      RecipientIdCache.INSTANCE.put(recipient);
      recipientId = recipient.getId();
    }
  ...
  //AA+
  public boolean isUfsrv() {
    return id == UFSRV_ID;
  }

  //AA++
  public static RecipientId requireUnknown() {
    return from (UNKNOWN_ID);
  }
