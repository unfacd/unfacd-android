...
public class UnidentifiedAccessUtil {
  ...
  public static List<Optional<UnidentifiedAccessPair>> getAccessFor(@NonNull Context context, @NonNull List<Recipient> recipients, boolean log) {
    ...
    for (Recipient recipient : recipients) {
      ...
      if (TextUtils.isEmpty(recipient.requireUfsrvUid())) {
        Log.d(TAG, String.format("getAccessFor: no ufsrvuid found for RecipientId:'%s'", recipient.getId().serialize()));
        return new LinkedList(){{add(Optional.empty());}};
      }

      if (recipient.requireUfsrvUid().equals(UfsrvUserUtils.myOwnUfsrvUid())) return new LinkedList(){{add(Optional.empty());}};// AA+ disabled for self
