...
public class SubmitDebugLogRepository {
  ...
  private static final String API_ENDPOINT     = "https://hastebin.com/documents";//AA++
  ...
  //AA for 'byte[] trace' see see https://github.com/signalapp/Signal-Android/commit/7bb1262571faa0ec622b1c023e6ac3220462b521
  @WorkerThread
  private @NonNull Optional<String> submitLogInternal(long untilTime, @NonNull List<LogLine> prefixLines, @Nullable byte[] trace) {
    ...
    try {
      ...
      request.setEntity(new StringEntity(prefixStringBuilder.toString()));
      ...
      try (LogDatabase.Reader reader = LogDatabase.getInstance(context).getAllBeforeTime(untilTime)) {
        while (reader.hasNext()) {
          sb.append(Scrubber.scrub(reader.next())).append("\n");
        }
      }
      ...
      if (element.has("key")) {
        ...
        if (!TextUtils.isEmpty(key)) {
          return Optional.of("https://hastebin.com" + "/" + key);//AA++
        }
      }

      Log.e(TAG, String.format("submitLogInternal: Could not submit log: '%s'", responseBody));//AA+
      return Optional.empty();
    ...
    //AA- see https://github.com/signalapp/Signal-Android/commit/7bb1262571faa0ec622b1c023e6ac3220462b521
    /*@WorkerThread
  private @NonNull Optional<String> submitLogInternal(long untilTime, @NonNull List<LogLine> prefixLines, @Nullable byte[] trace) {
    String traceUrl = null;
    if (trace != null) {
