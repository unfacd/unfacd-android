...
final class MessageDetailsRepository {
  ...
  //AA+ todo: implement
  /*private void updateReported(final MessageRecord messageRecord) {
    if (messageRecord.getUfsrtvStatus() == MessageReportedJob.MessageStatus.REPORTED.getValue()) {
      reportedContainer.setVisibility(View.VISIBLE);
      reportedInText.setText(getString(R.string.conversation_item_details_this_message_has_been_reported_as_inapporopriate));
    }
