...
public class AttachmentDatabase extends Database {
  ...
  public      static final String UFID       = "ufid";//AA+
...
//AA+
 private static final String PART_ID__WITH_UFID_WHERE = ROW_ID + " = ? AND " + UNIQUE_ID + " = ? AND " + UFID + " = ?";
  private static final String PART_ID_WITH_UFID_WHERE_NOT = ROW_ID + " != ? AND " + UNIQUE_ID + " != ? AND " + UFID + " != ?";
  ...
  public static final String[] CREATE_INDEXS = {
    ...
    "CREATE INDEX IF NOT EXISTS ufid_index ON " + TABLE_NAME + " (" + UFID + ");", //AA+
  };
  ...
  //AA+
  public @NonNull List<DatabaseAttachment> getAttachmentsForRowId(long rowID) {
    SQLiteDatabase           database = databaseHelper.getSignalReadableDatabase();
    List<DatabaseAttachment> results  = new LinkedList<>();
    Cursor                   cursor   = null;

    try {
      cursor = database.query(TABLE_NAME, PROJECTION, ROW_ID + " = ?", new String[] {rowID+""},
                              null, null, UNIQUE_ID + " ASC, " + ROW_ID + " ASC");
  ...
  public boolean deleteAttachmentsForMessage(long mmsId) {
    ...
    try {
      cursor = database.query(TABLE_NAME, new String[] {DATA, CONTENT_TYPE, ROW_ID, UNIQUE_ID, UNIQUE_ID, UFID}, MMS_ID + " = ?",//AA+
      ...
      while (cursor != null && cursor.moveToNext()) {
        deleteAttachmentOnDisk(CursorUtil.requireString(cursor, DATA),
                               ...
                               new AttachmentId(CursorUtil.requireLong(cursor, ROW_ID),
                                                ...
                                                CursorUtil.requireString(cursor, UFID))); //AA+
      }
    } finally {
      if (cursor != null)
        cursor.close();
  ...
  public int deleteAbandonedPreuploadedAttachments() {
    ...
    try (Cursor cursor = db.query(TABLE_NAME, null, query, args, null, null, null)) {
      while (cursor != null && cursor.moveToNext()) {
        ...
        String       ufId     = cursor.getString(cursor.getColumnIndex(UFID));
        AttachmentId id       = new AttachmentId(rowId, uniqueId, ufId);//AA+
  ...
  public void deleteAttachmentFilesForViewOnceMessage(long mmsId) {
    ...
    try {
      cursor = database.query(TABLE_NAME, new String[] {DATA, CONTENT_TYPE, ROW_ID, UNIQUE_ID, UFID}, MMS_ID + " = ?",//AA+
                              new String[] {mmsId+""}, null, null, null);
  ...
  private @NonNull DataUsageResult getAttachmentFileUsages(@Nullable String data, @NonNull AttachmentId attachmentId) {
    ...
    try (Cursor cursor = database.query(TABLE_NAME, new String[]{ROW_ID, UNIQUE_ID, QUOTE, UFID}, selection, args, null, null, null, null)) {//AA+ UFID param
      while (cursor.moveToNext()) {
        boolean isQuote = cursor.getInt(cursor.getColumnIndexOrThrow(QUOTE)) == 1;
        if (isQuote) {
          quoteRows.add(new AttachmentId(cursor.getLong(cursor.getColumnIndexOrThrow(ROW_ID)), cursor.getLong(cursor.getColumnIndexOrThrow(UNIQUE_ID)), cursor.getString(cursor.getColumnIndexOrThrow(UFID))));//AA+
        } else {
          ...
          throws MmsException
  {
    ...
    contentValues.put(UFID, sourceAttachment.getUfId()); //AA+
  ...
  public void updateAttachmentAfterUpload(@NonNull AttachmentId id, @NonNull Attachment attachment, long uploadTimestamp) {
    ...
    values.put(UFID, attachment.getUfId());//AA+

    if (dataInfo != null && dataInfo.hash != null) {
      updateAttachmentAndMatchingHashes(database, id, dataInfo.hash, values);
    } else {
      int i = database.update(TABLE_NAME, values, PART_ID_WHERE, id.toStrings());
      Log.w(TAG, String.format("updateAttachmentAfterUpload ('%d'): updated attachment UFID:'%s' ",  i, attachment.getUfId()));
    }
  ...
  @NonNull Map<Attachment, AttachmentId> insertAttachmentsForMessage(long mmsId, @NonNull List<Attachment> attachments, @NonNull List<Attachment> quoteAttachment)
      throws MmsException
  {
    Log.w(TAG, ">>insertAttachmentsForMessage: insertParts(" + attachments.size() + ")");
    ...
    for (Attachment attachment : attachments) {
      ...
      Log.w(TAG, String.format("Inserted attachment at DB ID:'%s'. UFID:'%s' ",  attachmentId, attachment.getUfId()));
    }
      ...
      if (cursor.getColumnIndex(AttachmentDatabase.ATTACHMENT_JSON_ALIAS) != -1) {
        ...
        for (int i=0;i<array.length();i++) {
          ...
          if (!object.isNull(ROW_ID)) {
            ...
            result.add(new DatabaseAttachment(new AttachmentId(object.getLong(ROW_ID), object.getLong(UNIQUE_ID), object.getString(UFID)), //AA+ Ufid
                                              ...
                                              object.getString(object.getString(UFID))));//AA+
          }
        }
  ...
  private @NonNull DatabaseAttachment getAttachment(@NonNull Cursor cursor) {
    ...
    return new DatabaseAttachment(new AttachmentId(cursor.getLong(cursor.getColumnIndexOrThrow(ROW_ID)),
                                                   ...
                                                   cursor.getString(cursor.getColumnIndexOrThrow(UFID))),//AA+ ufid
                                  ...
                                  cursor.getString(cursor.getColumnIndexOrThrow(UFID)));//AA+
  }
          ...
          throws MmsException
  {
    ...
    try {
      ...
      contentValues.put(UFID, attachment.getUfId());//AA+

      long rowId = database.insert(TABLE_NAME, null, contentValues);

      attachmentId = new AttachmentId(rowId, uniqueId, attachment.getUfId());//AA+
