...
public class PushDatabase extends Database {
  ...
  public long insert(@NonNull SignalServiceEnvelope envelope) {
    Optional<Long> messageId = find(envelope);//AA todo: do we ned to include ufrsv in the search criteria?
    ...
    if (messageId.isPresent()) {
      ...
      //AA+ populate ufsrv columns and defer type deserilisation until object is recalled from db in get(msgid)
...
//AA+  support to rebuid ufsrv columns into the reenacted Envelope object. We restore the whole blob
    ...
    try {
      ...
      if (cursor != null && cursor.moveToNext()) {
        ...
        return new SignalServiceEnvelope(cursor.getInt(cursor.getColumnIndexOrThrow(TYPE)),
                                         ...
                                         Util.isEmpty(ufsrvMessage) ? null : Base64.decode(ufsrvMessage),//AA+
                                         ufsrvmsgType);//AA+
      }
    } catch (IOException e) {
      Log.w(TAG, e);
    ...
    public SignalServiceEnvelope getNext() {
      try {
        ...
        return new SignalServiceEnvelope(type,
                                         ...
                                         serverGuid,
                                         ufsrvMessage != null ? Base64.decode(ufsrvMessage) : null,//AA+
                                         ufsrvMessageType);//AA+
      } catch (IOException e) {
