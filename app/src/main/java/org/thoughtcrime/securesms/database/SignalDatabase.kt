...
open class SignalDatabase(private val context: Application, databaseSecret: DatabaseSecret, attachmentSecret: AttachmentSecret) :
  ...
  //AA+
  val unfacdIntroContacts: UnfacdIntroContactsDatabase = UnfacdIntroContactsDatabase(context, this)
  ...
  override fun onCreate(db: net.zetetic.database.sqlcipher.SQLiteDatabase) {
    ...
    db.execSQL((UnfacdIntroContactsDatabase.CREATE_TABLE))//AA+
  ...
  companion object {
    ...
    private const val DATABASE_NAME = "unfacd.db"//AA+
    ...
    //AA+
    @get:JvmStatic
    @get:JvmName("unfacdIntroContacts")
    val unfacdIntroContacts: UnfacdIntroContactsDatabase
      get() = instance!!.unfacdIntroContacts
  }
}