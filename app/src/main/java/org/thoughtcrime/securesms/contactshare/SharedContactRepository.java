...
public class SharedContactRepository {
  ...
  private @Nullable Contact getContactFromSystemContacts(long contactId) {
    ...
    //    UfsrvUid    ufsrvUid     = getUfsrvUidInfo (contactId, phoneNumbers);//AA+ we cannot use phone numbers direct with UfsrcUid

    return new Contact(name, null, phoneNumbers, getEmails(contactId), getPostalAddresses(contactId), avatar, UfsrvUid.UndefinedUfsrvUid);//AA+
  }
  ...
  @WorkerThread
  private @Nullable
  UfsrvUid getUfsrvUidInfo(long contactId, List<Phone> phoneNumbers) {

    for (Phone phoneNumber : phoneNumbers) {
      Recipient recipient = Recipient.live(phoneNumber.getNumber()).get();
      if (recipient != null && recipient.getUfrsvUidRaw() != null) {
        return  new UfsrvUid(recipient.getUfrsvUidRaw());
      }
    }
    return null;
  }
