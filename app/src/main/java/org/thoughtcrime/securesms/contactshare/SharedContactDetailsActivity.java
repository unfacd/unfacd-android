  ...
  private void presentActionButtons(@NonNull List<RecipientId> recipients) {
    ...
    if (!pushUsers.isEmpty()) {
      ...
      callButtonView.setOnClickListener(v -> {
        ContactUtil.selectRecipientThroughDialog(this, pushUsers, dynamicLanguage.getCurrentLocale(), recipient -> CommunicationActions.startVoiceCall(this, recipient, Long.valueOf(-1)));//AA+ TODO: fix fid for voice contact
      });
