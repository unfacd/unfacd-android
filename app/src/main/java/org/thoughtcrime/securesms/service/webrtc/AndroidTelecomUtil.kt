...
object AndroidTelecomUtil {
  ...
  fun registerPhoneAccount() {
    if (Build.VERSION.SDK_INT >= 26 && !systemRejected) {
      ...
      val phoneAccount = PhoneAccount.Builder(getPhoneAccountHandle(), "unfacd")//AA+
...
//    return true // "${Build.MODEL} ${Build.PRODUCT} ${Build.MANUFACTURER}".contains(other = "oneplus", ignoreCase = true) //AA- see commit below
    return !FeatureFlags.internalUser() // https://github.com/signalapp/Signal-Android/commit/77ba6e0f7b11424d128806b6e75a20bd0e969e4d
  }
}
