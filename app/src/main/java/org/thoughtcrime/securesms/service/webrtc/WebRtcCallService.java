...
public final class WebRtcCallService extends Service implements SignalAudioManager.EventListener {
  ...
  private void registerNetworkReceiver() {
    if (networkReceiver == null) {
      ...
      ContextCompat.registerReceiver(getApplicationContext(), networkReceiver, new IntentFilter(ConnectivityManager.CONNECTIVITY_ACTION), ContextCompat.RECEIVER_EXPORTED);
    }
  }

  private void unregisterNetworkReceiver() {
    if (networkReceiver != null) {
      try {//AA+ getting random errors "Receiver not registered" on sending Hangup
        unregisterReceiver(networkReceiver);
      } catch (Exception e) {
        Log.e(TAG, "unregisterReceiver: caught Receiver not registered error", e);
      }
  ...
  public void registerPowerButtonReceiver() {
    if (!AndroidTelecomUtil.getTelecomSupported() && powerButtonReceiver == null) {
      ...
      ContextCompat.registerReceiver(getApplicationContext(), powerButtonReceiver, new IntentFilter(Intent.ACTION_SCREEN_OFF), ContextCompat.RECEIVER_EXPORTED);
    }
  }
