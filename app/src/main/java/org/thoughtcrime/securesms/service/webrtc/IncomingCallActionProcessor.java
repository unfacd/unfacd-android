...
public class IncomingCallActionProcessor extends DeviceAwareActionProcessor {
  ...
  protected @NonNull WebRtcServiceState handleAcceptCall(@NonNull WebRtcServiceState currentState, boolean answerWithVideo) {
    ...
    SignalDatabase.sms().insertReceivedCall(activePeer.getGroupFid(), activePeer.getId(), currentState.getCallSetupState(activePeer).isRemoteVideoOffer());//AA+ fid
  ...
  protected @NonNull WebRtcServiceState handleDenyCall(@NonNull WebRtcServiceState currentState) {
    ...
    try {
      ...
      SignalDatabase.sms().insertMissedCall(activePeer.getGroupFid(), activePeer.getId(), System.currentTimeMillis(), currentState.getCallSetupState(activePeer).isRemoteVideoOffer());//AA+ fid
