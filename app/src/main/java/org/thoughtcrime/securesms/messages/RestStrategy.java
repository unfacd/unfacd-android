...
public class RestStrategy extends MessageRetrievalStrategy {
  ...
  public boolean execute(long timeout) {
    ...
    try (IncomingMessageProcessor.Processor processor = ApplicationDependencies.getIncomingMessageProcessor().acquire()) {
      ...
      deleteServerEnqueuedMessage();//AA+ although there is an acknowledgement mechanism for individual messages via V1/Message/Gid endpoint, some queued messages persist, becoming stake and would be fetched everytime client rejoins the network, because "queue_size" in state sync message would be greater than '0'
      return true;
    } catch (IOException e) {
  ...
  //AA+
  private static int deleteServerEnqueuedMessage()
  {
    SignalServiceMessageReceiver receiver = ApplicationDependencies.getSignalServiceMessageReceiver();
    try {
      receiver.deleteEnqueuedMessages();
    } catch (IOException e) {
          ...
          throws IOException
  {
    ...
    receiver.retrieveMessages(envelope -> {
      ...
      Log.i(TAG, "Successfully attached retrieved envelope to PushDecryptMessageJob." + timeSuffix(startTime));
    });
