...
public class IncomingMessageProcessor {
  ...
  public class Processor implements Closeable {
     ...
     */
    public @Nullable
    String processEnvelope(@NonNull SignalServiceEnvelope envelope) {
      UfsrvUid ufsrvUidSender = UfsrvCommandUtils.getOriginatorUfsrvUserId(envelope.getUfsrvCommand());

      if (!ufsrvUidSender.isUndefined()) { //AA+
        Recipient recipient = Recipient.live(ufsrvUidSender.getUfsrvUidEncoded()).get();
        if (!isActiveNumber(recipient)) {
          //AA+
          ContactTokenDetails contactTokenDetails = new ContactTokenDetails();
          contactTokenDetails.setUfsrvUid(ufsrvUidSender.getUfsrvUidEncoded());
          //

          SignalDatabase.recipients().setRegistered(recipient.getId(), RecipientDatabase.RegisteredState.REGISTERED, contactTokenDetails);//AA+ contactToken
          ApplicationDependencies.getJobManager().add(new DirectoryRefreshJob(recipient, false, true));
        }
      }
      ...
      if (envelope.isReceipt()) {
        ...
        return null;
      } else if (envelope.isUfsrvMessage() || envelope.isPreKeySignalMessage() || envelope.isSignalMessage() || envelope.isUnidentifiedSender() || envelope.isPlaintextContent()) {//AA+ isUsfrvMessage
    ...
    private void processReceipt(@NonNull SignalServiceEnvelope envelope) {
      Recipient sender = Recipient.external(context, envelope.getSourceIdentifier());//AA+ //AA+ sourceIdentifier
    ...
    private boolean needsToEnqueueProcessing(@NonNull DecryptionResult result) {
      ...
      if (groupContext != null) {
        try {
          ...
          if (groupId.isV2()) {
            String        queueName     = PushProcessMessageJob.getQueueName(Recipient.externalGroupExact(context, groupId).getId());//AA+externalGroupExact
            ...
            return !jobManager.isQueueEmpty(queueName)                                                                   ||
                    ...
                    groupDatabase.getGroup(groupId.requireV2()).isPresent();//AA+ getGroup
          } else {
            return false;
          }
