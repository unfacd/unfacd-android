...
public class BackgroundMessageRetriever
{
  ...
  private static final String WAKE_LOCK_TAG  = "BackgroundMessageRetriever";
   ...
   */
  @WorkerThread
  public boolean retrieveMessages(@NonNull Context context, boolean isForceRun, MessageRetrievalStrategy... strategies) {
    return retrieveMessages(context, DO_NOT_SHOW_IN_FOREGROUND, isForceRun, strategies);//AA+ isForceRun
  }
   ...
   */
  @WorkerThread
  public boolean retrieveMessages(@NonNull Context context, long showNotificationAfterMs, boolean isForceRun, MessageRetrievalStrategy... strategies) {
    if (!isForceRun && shouldIgnoreFetch()) { //AA+ force run
    ...
    synchronized (this) {
      try (DelayedNotificationController controller = GenericForegroundService.startForegroundTaskDelayed(context, context.getString(R.string.BackgroundMessageRetriever_checking_for_messages), showNotificationAfterMs)) {
        ...
        try {
          ...
          return executeBackgroundRetrieval(context, startTime, strategies, isForceRun);//AA+ isForce
        }
        finally {
  ...
  private boolean executeBackgroundRetrieval(@NonNull Context context, long startTime, @NonNull MessageRetrievalStrategy[] strategies, boolean isForceRun) {
    boolean success = false;

    for (MessageRetrievalStrategy strategy : strategies) {
      if (!isForceRun && shouldIgnoreFetch()) {//AA+
