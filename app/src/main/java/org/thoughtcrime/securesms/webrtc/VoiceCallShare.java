        ...
        if (cursor != null && cursor.moveToNext()) {
          ...
          SimpleTask.run(() -> Recipient.external(this, destination), recipient -> {
            if (!TextUtils.isEmpty(destination)) {
              ApplicationDependencies.getSignalCallManager().startOutgoingAudioCall(recipient, -1);// .putExtra(WebRtcCallService.EXTRA_REMOTE_PEER, new RemotePeer(recipient.getId(), -1))//AA+
