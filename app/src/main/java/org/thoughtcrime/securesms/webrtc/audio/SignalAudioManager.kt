...
class FullSignalAudioManager(context: Context, eventListener: EventListener?) : SignalAudioManager(context, eventListener) {
  ...
  override fun initialize() {
    ...
    if (state == State.UNINITIALIZED) {
      ...
      ContextCompat.registerReceiver(context, wiredHeadsetReceiver, IntentFilter(if (Build.VERSION.SDK_INT >= 21) AudioManager.ACTION_HEADSET_PLUG else Intent.ACTION_HEADSET_PLUG), ContextCompat.RECEIVER_EXPORTED)
