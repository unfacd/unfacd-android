...
public class WebRtcCallActivity extends BaseActivity implements SafetyNumberChangeDialog.Callback {
  ...
  /*private void setGroupFid(@NonNull Recipient recipient) {
    GroupDatabase database = SignalDatabase.groups();
    Optional<GroupDatabase.GroupRecord> groupRecord = database.getPairedGroupWith(recipient);

    if (groupRecord.isPresent()) {
      viewModel.setGroupFid(groupRecord.get().getFid());
    }
  }*/
  ...
  private void handleAnswerWithAudio() {
    Permissions.with(this)
            ...
            .onAllGranted(() -> {
              ...
              ApplicationDependencies.getSignalCallManager().acceptCall(false);//intent.putExtra(EXTRA_FID, groupFid);//AA+
            })
    ...
    if (!recipient.equals(Recipient.UNKNOWN)) {
      Permissions.with(this)
              ...
              .onAllGranted(() -> {
                ...
                ApplicationDependencies.getSignalCallManager().acceptCall(true);//intent.putExtra(EXTRA_FID, groupFid);//AA+
  ...
  private void handleDenyCall() {
    ...
    if (!recipient.equals(Recipient.UNKNOWN)) {
      ApplicationDependencies.getSignalCallManager().denyCall();//intent.putExtra(EXTRA_FID, groupFid);//AA+
  ...
  private void handleEndCall() {
    ...
    ApplicationDependencies.getSignalCallManager().localHangup();//intent.putExtra(EXTRA_FID, groupFid);//AA+
  }
  ...
  public void onSendAnywayAfterSafetyNumberChange(@NonNull List<RecipientId> changedRecipients) {
    ...
    if (state.getGroupCallState().isConnected()) {
      ApplicationDependencies.getSignalCallManager().groupApproveSafetyChange(changedRecipients);//.putExtra(WebRtcCallService.EXTRA_REMOTE_PEER, new RemotePeer(viewModel.getRecipient().getId(), groupFid))//AA+
    } else {
  ...
  private void startCall(boolean isVideoCall) {
    ...
    if (isVideoCall) {
      ...
      ApplicationDependencies.getSignalCallManager().startOutgoingAudioCall(viewModel.getRecipient().get(), -1);
    }
