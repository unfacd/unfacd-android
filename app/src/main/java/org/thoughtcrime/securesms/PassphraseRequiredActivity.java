...
import com.unfacd.android.ui.TBDIntroActivity;
...
public abstract class PassphraseRequiredActivity extends BaseActivity implements MasterSecretListener {
  ...
  private static final int STATE_EXPERIENCE_UPGRADE  = 4;// TODO: 25/07/2024 to be removed. check https://github.com/signalapp/Signal-Android/commit/8add9ba0a6003e468252c7d512b11704af40f039
  private static final int STATE_WELCOME_PUSH_SCREEN = 5;
  private static final int STATE_ENTER_SIGNAL_PIN    = 6;
  private static final int STATE_CREATE_PROFILE_NAME = 7;
  private static final int STATE_CREATE_SIGNAL_PIN   = 8;
  private static final int STATE_TRANSFER_ONGOING    = 9;
  private static final int STATE_TRANSFER_LOCKED     = 10;
  private static final int STATE_CHANGE_NUMBER_LOCK  = 11;



  //AA+ intro screen
  private static final int STATE_INTRO_SCREEN             = 20;
  //
  ...
  //AA+
  protected ChildViewEventListener childViewEventListener = null;
  //
  ...
  protected void onResume() {
    ...
    //AA+
    if (!TextUtils.isEmpty(TextSecurePreferences.getUfsrvCookie(this))) {
      Log.d(TAG, ">>> onResume(): COOKIE SET -> INITIATING IncomingMessageObserver");
      ApplicationContext.getInstance().notifyIncomingMessageObserver(); //necessary for new registrations

      if (networkAccess.isCensored()) {
        ApplicationDependencies.getJobManager().add(new PushNotificationReceiveJob());
      }
    } else {
      Log.d(TAG, ">>> onResume(): NO COOKIE -> NOT INITIATING IncomingMessageObserver");
    }
    //
  }
  ...
  private Intent getIntentForState(int state) {
    ...
    switch (state) {
      ...
      case  STATE_INTRO_SCREEN:       return getPushRegistrationIntent();//getIntroScreenIntent(); //AA+
  ...
  private int getApplicationState(boolean locked) {
    ...
    if (!MasterSecretUtil.isPassphraseInitialized(this)) {
    ...
    } else if (!TextSecurePreferences.getUfsrvUfAccountCreated(this) && !TextSecurePreferences.getBooleanPreference(this, TextSecurePreferences.UFSRV_SEEN_INTRO, false)) {
      Log.d(TAG, ">>> getApplicationState(): Account not created: INTRO SCREEN NOT SEEN... launching INTRO");
      return STATE_INTRO_SCREEN;
    } else if (TextUtils.isEmpty(TextSecurePreferences.getUfsrvUserId(this)) || !TextSecurePreferences.getUfsrvUfAccountCreated(this)) {//AA+
      Log.d(TAG, ">>> getApplicationState(): Account not created... launching registration");
      ...
      Log.d(TAG, ">>> getApplicationState(): ALL GOOD: No special state requirements: launching NORMAL...");
      return STATE_NORMAL;
    }
  }
...
//    return getRoutedIntent(RegistrationActivity.class, getCreateProfileIntent());//AA orig temporarily disabled

    return RegistrationNavigationActivity.newIntentForNewRegistration(this, getIntent());//AA- todo: enable when new registration is ported
  }
  ...
  private Intent getIntroScreenIntent() {
    return getRoutedIntent(TBDIntroActivity.class, getConversationListIntent());
  }
  ...
  private Intent getRoutedIntent(Class<?> destination, @Nullable Intent nextIntent) {
    ...
    intent.putExtra(EditProfileActivity.SHOW_TOOLBAR, false);//AA+
...
//AA+ todo decouple from this Activity
  public ChildViewEventListener getChildViewEventListener() {
    return childViewEventListener;
  }
  ...
  //AA+
  public void setChildViewEventListener (ChildViewEventListener listener)
  {
    this.childViewEventListener = listener;
  }

  public interface ChildViewEventListener
  {
    void onUfsrvCommandEvent (UfsrvCommandEvent event);
  }
  //
}
