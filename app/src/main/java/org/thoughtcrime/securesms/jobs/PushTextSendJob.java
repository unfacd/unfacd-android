          ...
          throws UntrustedIdentityException, InsecureFallbackApprovalException, UndeliverableMessageException, IOException
  {
    try {
      ...
      if (address.getNumber().equals(TextSecurePreferences.getUfsrvUserId(context))) {//AA+ usfrv
        ...
        SignalLocalMetrics.IndividualMessageSend.onDeliveryStarted(messageId);
        SendMessageResult result = messageSender.sendDataMessage(address, unidentifiedAccess, ContentHint.RESENDABLE, textSecureMessage, new MetricEventListener(messageId), null, null, new UfsrvCommand(messageCommand, false, false));//AA+
