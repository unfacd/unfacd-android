...
public class ReactionSendJob extends BaseJob {
  ...
  public @NonNull Data serialize() {
    return new Data.Builder().putLong(KEY_MESSAGE_ID, messageId.getId())
                             ...
                             .putLong(KEY_FID, reaction.getFid())//AA+
                             .build();
  }
  ...
  protected void onRun() throws Exception {
    ...
    try { //AA+ try
      ...
      Log.i(TAG, "Completed now: " + completions.size() + ", Remaining: " + recipients.size() +" IGNORE REMAINING DUE TO UFSRV FAN OUT");

      //AA- ufsrv does fan out of reaction, client doesn't track individually. Safe to do so, as the remaining recipients are just serialised with the job when retry exception is raised.
//      if (!recipients.isEmpty()) {
//        Log.w(TAG, "Still need to send to " + recipients.size() + " recipients. Retrying.");
//        throw new RetryLaterException();
//      }
    } catch (EncapsulatedExceptions x) {
      Log.e(TAG, String.format("onRun(): %s", x.getMessage()));
    }
  }
  ...
  private @NonNull List<Recipient> deliver(@NonNull Recipient conversationRecipient, @NonNull List<Recipient> destinations, @NonNull Recipient targetAuthor, long targetSentTimestamp)
          throws IOException, UntrustedIdentityException, InvalidKeyException, EncapsulatedExceptions
  {
    long  timestamp = System.currentTimeMillis();//AA+

    SignalServiceDataMessage.Builder dataMessageBuilder = SignalServiceDataMessage.newBuilder()
                                                                                  .withTimestamp(timestamp)//AA+ timestamp
                                                                                  .withReaction(buildReaction(context, reaction, remove, targetAuthor, targetSentTimestamp));

    if (conversationRecipient.isGroup()) {
      GroupUtil.setDataMessageGroupContext(context, dataMessageBuilder, conversationRecipient.requireGroupId().requirePush(), conversationRecipient.getUfsrvId());//AA+ ufsrvid as fid
    }
    ...
    if (false && conversationRecipient.isPushV2Group()) {//AA+ false SenderKey not ported
      results     = GroupSendUtil.sendResendableDataMessage(context,
      ...
      //AA+
      MessageCommand.Builder messageCommandBuilder = buildMessageCommandForReaction(timestamp, MessageCommand.CommandTypes.REACTION_VALUE, 0, conversationRecipient.getUfsrvId());
      addresses.add(RecipientUtil.toSignalServiceAddress(context, targetAuthor));//AA+ this is just mock, as target list is based on fid

      results = messageSender.sendDataMessage(unidentifiedAccess, false, ContentHint.DEFAULT, dataMessage, addresses, SignalServiceMessageSender.IndividualSendEvents.EMPTY, new UfsrvCommand(messageCommandBuilder, false, UfsrvCommand.TransportType.API_SERVICE, false));//AA+
//      SignalDatabase.messageLog().insertIfPossible(dataMessage.getTimestamp(), destinations, results, ContentHint.RESENDABLE, messageId, isMms);//AA to SenderKey https://github.com/signalapp/Signal-Android/commit/f19033a7a2073c09b11d6c613cb0fcff00bfef36
    }
  ...
  buildMessageCommandForReaction (long timeSentInMillis, int command, int commandArg, long fid)
  {
