...
public final class IdentityUtil {
  ...
  public static void markIdentityVerified(Context context, Recipient recipient, boolean verified, boolean remote)
  {
    ...
    try (GroupDatabase.Reader reader = groupDatabase.getActiveGroups()) {//AA+ ActiveGroups

      GroupDatabase.GroupRecord groupRecord;

      while ((groupRecord = reader.getNext()) != null) {
        if (groupRecord.getMembersRecipientId().contains(recipient.getId()) && groupRecord.isActive() && !groupRecord.isMms()) {//AA+ getMembersRecipientId
  ...
  public static void markIdentityUpdate(@NonNull Context context, @NonNull RecipientId recipientId) {
    ...
    try (GroupDatabase.Reader reader = groupDatabase.getGroups()) {
      ...
      while ((groupRecord = reader.getNext()) != null) {
        if (groupRecord.getMembers().contains(recipientId) && groupRecord.isActive()) {
          IncomingTextMessage           incoming    = new IncomingTextMessage(recipientId, 1, time, time, time, null, Optional.of(groupRecord.getId()), 0, false, null, null);
    ...
    IncomingTextMessage           incoming         = new IncomingTextMessage(recipientId, 1, time, -1, time, null, Optional.empty(), 0, false, null, null);
  ...
  //AA+
  public static void saveIdentity(ContactTokenDetails tokenDetails)
  {
    try {
      if (TextUtils.isEmpty(tokenDetails.getIdentityKey())) {
        Log.w(TAG, "Identity key is missing on ContactTokenDetrails!");
        return;
      }

      IdentityKey identityKey = new IdentityKey(Base64.decode(tokenDetails.getIdentityKey()), 0);

      //see RetrieveProfileJob.setIdentityKey
      ...
      IdentityUtil.saveIdentity(tokenDetails.getUuid(), identityKey);
