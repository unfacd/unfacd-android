...
public class CommunicationActions {
  ...
  public static void startVoiceCall(@NonNull FragmentActivity activity, @NonNull Recipient recipient, @NonNull Long fid) { //AA+ fid
    ...
    if (recipient.isRegistered()) {
      ApplicationDependencies.getSignalCallManager().isCallActive(new ResultReceiver(new Handler(Looper.getMainLooper())) {
        ...
        protected void onReceiveResult (int resultCode, Bundle resultData) {
          if (resultCode == 1) {
            startCallInternal(activity, recipient, resultCode != 1, fid);//AA+ fid
          } else {
            new AlertDialog.Builder(activity)
                    .setMessage(R.string.CommunicationActions_start_voice_call)
                    .setPositiveButton(R.string.CommunicationActions_call, (d, w) -> startCallInternal(activity, recipient, resultCode != 1, fid))
  ...
  public static void startVideoCall(@NonNull FragmentActivity activity, @NonNull Recipient recipient, @NonNull Long fid) { //AA+ fid
    if (TelephonyUtil.isAnyPstnLineBusy(activity)) {
      Toast.makeText(activity,
                     R.string.CommunicationActions_a_cellular_call_is_already_in_progress,
                     Toast.LENGTH_SHORT)
    ...
    ApplicationDependencies.getSignalCallManager().isCallActive(new ResultReceiver(new Handler(Looper.getMainLooper())) {
      ...
      protected void onReceiveResult(int resultCode, Bundle resultData) {
        if (resultCode == 1) {
          startCallInternal(activity, recipient, false, fid);//AA+
        } else {
          new AlertDialog.Builder(activity)
                  .setMessage(R.string.CommunicationActions_start_video_call)
                  .setPositiveButton(R.string.CommunicationActions_call, (d, w) -> startCallInternal(activity, recipient, true, fid))//AA+
                  .setNegativeButton(R.string.CommunicationActions_cancel, (d, w) -> d.dismiss())
                  .setCancelable(true)
                  .show();
        }
                                        ...
                                        @NonNull GroupInviteLinkUrl groupInviteLinkUrl)
  {
    ...
    SimpleTask.run(SignalExecutors.BOUNDED, () -> {
                     ...
                     return group != null && group.isActive() && group.getMode() != GROUP_MODE_LINKJOIN_REQUESTING ? Recipient.resolved(group.getRecipientId())//AA+ getMode()
                                                                                                                   : null;
                   },
                   recipient -> {
                     if (recipient != null) {
  ...
  private static void startCallInternal(@NonNull FragmentActivity activity, @NonNull Recipient recipient, boolean isVideo, long fid) {//AA+ fid
    if (isVideo) startVideoCallInternal(activity, recipient, fid);
    else         startAudioCallInternal(activity, recipient, fid);
  }

  private static void startAudioCallInternal(@NonNull FragmentActivity activity, @NonNull Recipient recipient, long fid) {//AA+ fid
    Permissions.with(activity)
               ...
               .onAllGranted(() -> {
                 ApplicationDependencies.getSignalCallManager().startOutgoingAudioCall(recipient, fid);//.putExtra(WebRtcCallService.EXTRA_REMOTE_PEER, new RemotePeer(recipient.getId(), fid))//AA+ fid
                 MessageSender.onMessageSent();
                 Intent activityIntent = new Intent(activity, WebRtcCallActivity.class);
                 activityIntent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//                 activityIntent.putExtra(WebRtcCallService.EXTRA_FID, fid);//AA+
                 activity.startActivity(activityIntent);
               })
               .execute();
  }

  private static void startVideoCallInternal(@NonNull FragmentActivity activity, @NonNull Recipient recipient, long fid) {//AA+ fid
    Permissions.with(activity)
               ...
               .onAllGranted(() -> {
//                 .putExtra(WebRtcCallService.EXTRA_REMOTE_PEER, new RemotePeer(recipient.getId(), fid))//AA+
//                 .putExtra(WebRtcCallService.EXTRA_FID, fid);//AA+
                 ApplicationDependencies.getSignalCallManager().startPreJoinCall(recipient, fid);
