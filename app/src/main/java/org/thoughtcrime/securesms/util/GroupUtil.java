...
public final class GroupUtil {
          ...
          throws BadGroupIdException
  {
    if (groupContext.getGroupV1().isPresent()) {
      return GroupId.v2orThrow(groupContext.getGroupV1().get().getGroupId());//AA++ V2orThrow
  ...
  Optional<GroupId> idFromGroupContext(@NonNull Optional<SignalServiceGroupContext> groupContext)
          throws BadGroupIdException
  {
  ...
  @WorkerThread
  public static Optional<OutgoingGroupUpdateMessage> createGroupLeaveMessage(@NonNull Recipient groupRecipient) {
    ...
    GroupDatabase.GroupRecord groupRecord  = SignalDatabase.groups().getGroupByGroupId(encodedGroupId);
    ...
    GroupContext groupContext = GroupContext.newBuilder()
                                            ...
                                            .setFenceMessage(fenceCommand)//AA+
                                            .build();

    return Optional.of(new OutgoingGroupUpdateMessage(groupRecipient, groupContext, null, timeSentInMillis, 0, false, null, Collections.emptyList(), Collections.emptyList(), Collections.emptyList(),
                                                      ufsrvCommandBuilder.build()));//AA+
  }

  public static @NonNull
  GroupMasterKey requireMasterKey(@NonNull byte[] masterKey) {
    try {
...
//      dataMessageBuilder.asGroupMessage(group);
      ...
      dataMessageBuilder.asGroupMessage(new SignalServiceGroup(groupId.getDecodedId(), fid, groupRecord.getGroupMasterKey().orElse(null)));//AA+ fid, groupkey
    } else {
      dataMessageBuilder.asGroupMessage(new SignalServiceGroup(groupId.getDecodedId(), fid, null));//AA+ fid
    }
  }
