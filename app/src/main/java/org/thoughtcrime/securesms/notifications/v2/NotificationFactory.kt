...
object NotificationFactory {
    ...
    //AA+
    fun notifyIntroMessage(context: Context, introId: Long) {
        val builder: NotificationBuilder = NotificationBuilder.createIntroContact(context, introId)
        val intent = Intent(context, ConversationListFragment::class.java)
        intent.putExtra(ConversationListFragment.INTROCONTACT_MSGID, introId)
        ...
        builder.apply {
            setSmallIcon(R.drawable.ic_notification)
            setLargeIcon(BitmapFactory.decodeResource(context.resources, R.drawable.ic_info_outline))
            setContentTitle(context.getString(R.string.IntroContactNotificationBuilder_intro_contact))
            setContentText(context.getString(R.string.IntroContactNotificationBuilder_intro_contact))
            setTicker(context.getString(R.string.IntroContactNotificationBuilder_intro_contact))
            setContentIntent(PendingIntent.getActivity(context, 0, intent, PendingIntentFlags.mutable()))
            setAutoCancel(true)
...
//        val builder = IntroContactNotificationBuilder(context, SignalStore.settings().messageNotificationsPrivacy, introId, intent)// old legacy
        (context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager)
          .notify(NotificationIds.UF_INTROMSG, builder.build())
    }
