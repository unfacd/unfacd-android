...
public class SwipeSelector extends FrameLayout {
    ...
    private boolean swipeLocked = false;//+AA
    ...
    public void setLocked (boolean swipeLocked)
    {
        this.swipeLocked = swipeLocked;
        adapter.setSwipeLocked(swipeLocked);
        ((WrappingPager)pager).setSwipeLocked(swipeLocked);
        if (swipeLocked) {
            if (leftButton!=null) {
                leftButton.setVisibility(INVISIBLE);
            }
    ...
    private void populateAttrsAndInitAdapter(Context context, AttributeSet attrs) {
        ...
        try {
            swipeLocked = ta.getBoolean(R.styleable.SwipeSelector_swipe_locked, false);//AA+
