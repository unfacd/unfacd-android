...
class SwipeAdapter extends PagerAdapter implements View.OnClickListener, ViewPager.OnPageChangeListener {
    ...
    private SwipeAdapter(Builder builder) {
        ...
        setSwipeLocked(builder.swipeLocked);
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            leftButton.getDrawable().setAutoMirrored(true);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            Configuration config = context.getResources().getConfiguration();
            if (config.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL) {
                leftButton.setImageDrawable(flip(builder.leftButtonResource));
            }
        } else {
            // Not supported
        }
        ...
        //AA+ RTL support
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            rightButton.getDrawable().setAutoMirrored(true);
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
            Configuration config = context.getResources().getConfiguration();
            if (config.getLayoutDirection() == View.LAYOUT_DIRECTION_RTL) {
                    rightButton.setImageDrawable(flip(builder.rightButtonResource));
                }
        } else {
            // Not supported
        }//
    ...
    void setSwipeLocked (boolean swipeLocked) {
      this.swipeLocked = swipeLocked;
      ((WrappingPager)viewPager).setSwipeLocked(swipeLocked);
    }
    ...
    private BitmapDrawable flip(int buttonRes) {
        Matrix m = new Matrix();
        m.preScale(-1, 1);
        Bitmap src = BitmapFactory.decodeResource(context.getResources(), buttonRes);
    ...
    static class Builder {
        ...
        Builder swipeLocked(boolean swipeLocked) {
          this.swipeLocked = swipeLocked;
          return this;
        }
