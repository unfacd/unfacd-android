 ...
 */
public class ApplicationContext extends androidx.multidex.MultiDexApplication implements AppForegroundObserver.Listener {

  private PersistentLogger persistentLogger;

  private IncomingMessageObserver  incomingMessageObserver;//AA+

  private EventBus ufsrvcmdEvents; //AA+
  ...
  public void onCreate() {
    ...
    ufsrvcmdEvents = EventBus.builder().build(); //AA+
    ...
    AppStartup.getInstance().addBlocking("security-provider", this::initializeSecurityProvider)
              ...
              .addNonBlocking(IntegrityUtils::initialiseIntegrityApi)//AA+
    ...
    //AA+
    initialiseStetho();
    initialiseCrashAnalytics();

    retrieveAppSignatureHashcode();//AA+
  ...
  public EventBus getUfsrvcmdEvents()
  {
    return ufsrvcmdEvents;
  }
  ...
  public void notifyIncomingMessageObserver() { //AA+ signalling mechanism to kick off websock connection
    incomingMessageObserver=  ApplicationDependencies.getIncomingMessageObserver();
    if (incomingMessageObserver != null) {
      incomingMessageObserver.onConstraintMet("");
    } else {
      Log.e(TAG, "ERROR: Could not assign incomingMessageObserver: Won't be able to wake up WebSocket connections if dormant");
    }
  }
  ...
  protected void attachBaseContext(Context base) {
    ...
    androidx.multidex.MultiDex.install(this);//AA+
  }

  private static class ProviderInitializationException extends RuntimeException {
  }

  //AA+ provides basic credentials across the app
  public static class DynamicCredentialsProvider implements CredentialsProvider
  {
    ...
    public String getPassword() {
      return SignalStore.account().getServicePassword();//.getPushServerPassword(context);
    }
    ...
    public int getDeviceId() {
      return SignalServiceAddress.DEFAULT_DEVICE_ID;
    }
  ...
  //AA+ provides credentials pre confirmed registration where credentials are loaded differently
  public static class RegistrationPendingCredentialsProvider implements CredentialsProvider
  {
    private final Context context;

    public RegistrationPendingCredentialsProvider() {
      this.context = ApplicationContext.instance;
    ...
    //AA+
    @Override
    public String getCookie() {return TextSecurePreferences.getUfsrvPendingCookie(context);}
