...
{
  private static final String TAG = Log.tag(MultiSelectPreferenceListGroupMembersSelected.class);
  ...
  {
    List<Recipient> recipientsMembers = SignalDatabase.recipients()
            .getMembersWithPermission(context,
                                      Recipient.live(Long.valueOf(groupId)).get(),
                                      FencePermissions.values()[selectedCriterion],
                                      false);
  ...
  {
    List<Recipient> recipientMembers=SignalDatabase.recipients()
                                                      .getMembersWithPermission(context,
                                                                                Recipient.live(Long.valueOf(groupId)).get(),
                                                                                FencePermissions.values()[selectedCriterion],
                                                                                false);
