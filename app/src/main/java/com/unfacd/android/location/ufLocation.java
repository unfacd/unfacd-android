...
{
  static private final String TAG = Log.tag(ufLocation.class);
  ...
  {
    //fully qualified: country:admin:locality:username:
  ...
  public static Optional<String> makeHomebaseLocationPrefix(Address addressIn, int stopMark)
  {
    //fully qualified: lat,long:country:admin:locality:user_zone:

    //priority given to client's view
    Address myA = addressIn;
    ...
    if (stopMark == 1) {
      ...
      return Optional.of(addressIn.getLatitude() + "," + addressIn.getLongitude() + ":" + baseLocationPrefixx);
    }//x::::
    ...
    //input: 'x:y:z:' or 'x::z:' or 'x:::'
    if (selfZoned) {
      return Optional.of( addressIn.getLatitude() + "," + addressIn.getLongitude() + ":" + baseLocationPrefixx + selfZoneName + ":");
    }
    else
      return Optional.of( addressIn.getLatitude() + "," + addressIn.getLongitude() + ":" + baseLocationPrefixx + "0:");

  }

//  /fully qualified: lat,long:country:admin:locality:user_zone:
  public static Optional<String> packHomebaseLocation(@NonNull String latlong, @NonNull String address)
  {
    if (!TextUtils.isEmpty(latlong) && !TextUtils.isEmpty(address)) {
      return Optional.of(latlong + ":" + address);
...
// Multiplexing views
  ...
  {
    if (inLocation != null) {
      return String.format(Locale.getDefault(), "Latitude %.6f, Longitude %.6f", inLocation.getLatitude(), inLocation.getLongitude());
    }
    return null;
  }
  ...
  public synchronized String describeCurrentLocation()
  {
    if (currentLocation != null) {
      return String.format(Locale.getDefault(), "Latitude %.6f, Longitude %.6f", currentLocation.getLatitude(), currentLocation.getLongitude());
    }
  ...
  public synchronized String describeLastLocation()
  {
    if (lastLocation != null) {
      return String.format(Locale.getDefault(), "%.6f,%.6f", lastLocation.getLatitude(), lastLocation.getLongitude());
    }

    return null;
  }

  public static Optional<String> describeLocation(@Nullable Location location)
  {
    if (location != null) {
      return Optional.of(String.format(Locale.getDefault(), "%.6f,%.6f", location.getLatitude(), location.getLongitude()));
    }
  ...
  unpackAddressFromHomeBaseLoc(@NonNull String homebaseLocPacked)
  {
    if (!TextUtils.isEmpty(homebaseLocPacked)) {
      int idx= homebaseLocPacked.indexOf(":");
      return (homebaseLocPacked.substring(idx + 1));
    }
  ...
  public static void
  unpackAddressFromHomeBaseLoc(@NonNull String homebaseLocPacked, Consumer<String> consumer)
  {
    if (!TextUtils.isEmpty(homebaseLocPacked)) {
      String unpacked[] = homebaseLocPacked.split(":", 1);
      consumer.accept(unpacked[1]);
    }
  }

  public static void
  unpackLatLongFromHomeBaseLoc(@NonNull String homebaseLocPacked, Consumer<String> consumer)
  {
    if (!TextUtils.isEmpty(homebaseLocPacked)) {
      String unpacked[] = homebaseLocPacked.split(":", 1);
      consumer.accept(unpacked[0]);
    }
  }

  /**
   *
   * @param homebaseLoc comes in as country:admin:locality:username:0000
   * @return country, admin, locality
   */
  public static String describeLocationFromHomebaseLoc(@NonNull String homebaseLoc)
  {
    if (!TextUtils.isEmpty(homebaseLoc)) {
      String trimmed = homebaseLoc.replaceAll(":\\d*:$", ""); //remove the user zone part from the end of the string represented by numbers
   ...
   */
  public static Location provideLatLongLocation(@NonNull String location)
  {
    try {
      int idx = location.indexOf(",");
      Location latlongRaw = new Location("");
      latlongRaw.setLatitude(Float.parseFloat(location.substring(0, idx)));
      latlongRaw.setLongitude(Float.parseFloat(location.substring(idx + 1)));

      return latlongRaw;
    } catch(Exception e) {
      Log.w(TAG, String.format("provideLatLongLocation: Malformed location: '%s'", location));
    }
  ...
  {
    if (geofence != null) {
      Log.i(TAG, ">>uf Location Geofencing:" + getTransitionNameFromType(transitionType) + " for Geofence with id = " + geofence.getRequestId());
    } else {
