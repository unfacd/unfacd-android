...
{
  private static final String TAG = Log.tag(MapableThingProvider.class);
  ...
  static public MapableThing getMapbleGroup(Context context, SignalServiceProtos.FenceRecord fenceRecord)
  {
    ...
    if (fenceRecord.hasAvatar() && fenceRecord.getAvatar().hasDigest()) {
      ...
      future.ifPresent(bitmapListenableFutureTask -> bitmapListenableFutureTask.addListener(new FutureTaskListener<>()
      {
        @Override
