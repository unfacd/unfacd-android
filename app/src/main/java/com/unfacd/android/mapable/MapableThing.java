...
{
  private static final String TAG = Log.tag(MapableThing.class);
  ...
  private final Set<MapableThing.MapableThingModifiedListener> listeners = Collections.newSetFromMap(new WeakHashMap<>());
...
//          .createForSingleSessionOnDisk(requireContext(), e -> Log.w(TAG, "Failed to persist image.", e));
  Optional<ListenableFutureTask<Bitmap>> BitmapFromAttachmentPointer(long fid, SignalServiceProtos.FenceRecord fenceRecord)
  {
    ...
    if (storedAvatar != null && storedAvatar.exists()) {
      try {
        ...
        Log.e(TAG, String.format(Locale.getDefault(), "BitmapFromAttachmentPointer (fid:'%d', id:'%s'): Using existing cached group avatar ", fid, fenceRecord.getAvatar().getId()));
    ...
    if (fenceRecord.getAvatar().getKey() != null && fenceRecord.getAvatar().getKey().size() == 64) {
      ApplicationDependencies.getJobManager().add(new MapableAvatarDownloadJob(fid, Base64.encodeBytes(fenceRecord.toByteArray())));//this.avatarPointer.asPointer()));
    } else {
      Log.e(TAG, String.format(Locale.getDefault(), "BitmapFromAttachmentPointer (fid:'%d', id:'%s'): Attachment has corrupt key ", fenceRecord.getFid(), fenceRecord.getAvatar().getId()));
    }
  ...
  public interface MapableThingModifiedListener {
    void onModified(MapableThing mapableThing);
  }
}
