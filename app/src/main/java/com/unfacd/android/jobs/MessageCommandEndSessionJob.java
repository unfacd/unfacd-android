...
public class MessageCommandEndSessionJob extends BaseJob {
  ...
  private static final String TAG = Log.tag(ResetGroupsJob.class);
  ...
  public void onRun() throws IOException
  {
    ...
    try {
      ...
      SmsMessageRecord record = database.getSmsMessage(this.messageId);
      Optional<Recipient>otherRecipient = resolveUid(record.getRecipient().requireAddress().serialize());
  ...
  UserCommand.Builder buildEndSessionUserCommand  (Recipient recipient)
  {
    ...
    userRecordBuilder.setUsername(recipient.requireAddress().toString());
  ...
  {
    Address myAdress = Address.fromSerialized(TextSecurePreferences.getUfsrvUserId(context));//AA+ uid
    try {
      Optional<Address> addressOther = SignalDatabase.groups().PairedGroupGetOther(GroupId.parse(address), myAdress, true);
      if (addressOther.isPresent()) {
        return Optional.ofNullable(Recipient.live(addressOther.get().serialize()).get());
      }
