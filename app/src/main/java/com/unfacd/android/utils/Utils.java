...
public class Utils
{
  ...
  {
    if (unread > 0) {
      ActionItemBadge.update(activity, itemBadgeUnread, CommunityMaterial.Icon.cmd_eye_off_outline, ContextCompat.getColor(activity, R.color.signal_icon_tint_primary), badgeStyleUnread, unread);
    } else {
...
//    LOG.debug("Compressed: " + output.length / 1024 + " Kb");
  ...
  if (!SignalStore.account().isRegistered()) return false;
  if (recipient.isGroup())                  return false;

  return Util.isOwnNumber(context, recipient.getUfsrvUid());
}
  ...
  public static String hexSerialise(byte[] hash)
  {
