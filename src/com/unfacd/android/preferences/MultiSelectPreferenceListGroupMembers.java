...
{
  private static final String TAG = MultiSelectPreferenceListGroupMembers.class.getSimpleName();
  ...
  private void fetchGroupMembers (Context context)
  {
    ...
    if (groupId!=null)
    {
      List<Recipient> recipientsGroupMembers=DatabaseFactory.getGroupDatabase(context)
              .getGroupMembers(Long.valueOf(groupId), false);

      recipientGroupMembers=recipientsGroupMembers;

      groupOwneruserId=DatabaseFactory.getGroupDatabase(context).getOwnerUserId (Long.valueOf(groupId));
    }
