...
final class Encoder {
  ...
  public StringBuilder encode(final BigInteger v, final StringBuilder accum) {
    ...
    final IntStream digits = IntStream.ofCodePoints(vs)
            .map(c -> Character.digit(c, RADIX)).map(i -> this.lookup[i]);

    return IntStream.concat(neg ? IntStream.of('-') : IntStream.empty(), digits)
            .collect(() -> accum, StringBuilder::appendCodePoint);
