...
public class RegistrationActivity extends BaseActionBarActivity implements VerificationCodeView.OnCodeEnteredListener,
        CheckingAccountVerificationStatusView.CountdownStateListener,//
...
//      goToContent();
  ...
  private void initializeResources() {
    ...
    this.registrationState    = new RegistrationState(RegistrationState.State.INITIAL, null, null, Optional.absent(), Optional.absent(), null, null, null, -1, null);
  ...
  private void initializeNumber() {
    ...
    if (localNumber.isPresent()) {
      ...
      if (simCountryIso.isPresent() && !TextUtils.isEmpty(simCountryIso.get())) {
        ...
        requestPhoneNumberHint();// doesnt work
      }
    }
  }
...
//        e164number = unspecifiedE164Number; //we dont return since it is optional
  ...
  private void handlePlayServicesCheck () {
    ...
    if (gcmStatus == PlayServicesStatus.SUCCESS) {
      handleRequestVerification(e164number, true, email.getText().toString());//
  ...
  private void handleRequestVerification(@NonNull String e164number, boolean gcmSupported, @NonNull String email) {
    ...
    if (false && gcmSupported && !TextUtils.isEmpty(e164number)) { // 2nd conditional
  ...
  private void requestVerificationCode(@NonNull String e164number, boolean gcmSupported, boolean smsRetrieverSupported, @NonNull String email) {
    ...
    new AsyncTask<Void, Void, RegistrationState> () {// RegistrationState
      @Override
        protected @Nullable RegistrationState doInBackground(Void... voids) { //
        try {
          markAsVerifying(true);

          String password = Util.getSecret(18);

          // set it too early for now because we may need access services/mastersecrete before registration is complete
          ...
          accountManager = AccountManagerFactory.createManager(RegistrationActivity.this, email/*e164number*/, password);
          ...
          String cookiePending = accountManager.requestSmsVerificationCode(smsRetrieverSupported,  registrationState.captchaToken, pushChallenge, Optional.of(e164number));// return value
          if (TextUtils.isEmpty(cookiePending)) throw new IOException();//

          TextSecurePreferences.setUfsrvPendingCookie(RegistrationActivity.this, cookiePending);
          return new RegistrationState(RegistrationState.State.INITIAL, e164number, password, fcmToken, Optional.absent(), cookiePending, null, email, -1, null);//

        } catch (IOException e) {
          Log.w(TAG, "Error during account registration", e);
          RegistrationState registrationState = new RegistrationState(RegistrationState.State.EXCEPTION, null, null, Optional.absent(), Optional.absent(), null, null, null, -1, null);
          registrationState.setException(Optional.of(e));
          return registrationState;
        }
      }

      protected void onPostExecute(@Nullable RegistrationState state) {//
        ...
        registrationState = new RegistrationState(RegistrationState.State.VERIFYING, state);//
  ...
  private void verifyAccount(@NonNull String code, @Nullable String pin) throws IOException {
    ...
    byte[]  profileKey                  = ProfileKeyUtil.getProfileKey(RegistrationActivity.this);//
    ...
    // return
    JsonEntityVerifiedAccount verifiedAccount=accountManager.verifyAccountWithCode(code, null, registrationId, !registrationState.gcmToken.isPresent(), pin, unidentifiedAccessKey, universalUnidentifiedAccess, registrationState.pendingCookie, registrationState.username, registrationState.e164number, profileKey);
    ...
    TextSecurePreferences.setUfsrvCookie(RegistrationActivity.this, registrationState.verifiedCookie);
    TextSecurePreferences.setUfsrvUsername(RegistrationActivity.this, registrationState.username);
    TextSecurePreferences.setUfsrvUfAccountCreated(RegistrationActivity.this, true);
    if (registrationState.accountUserid>0)  TextSecurePreferences.setUserId(RegistrationActivity.this, registrationState.accountUserid);
    if (!TextUtils.isEmpty(registrationState.ufsrvUid)) TextSecurePreferences.setUfsrvUserId(RegistrationActivity.this, registrationState.ufsrvUid);
    if (!TextUtils.isEmpty(registrationState.accessToken)) TextSecurePreferences.setAccessToken(RegistrationActivity.this, registrationState.accessToken);
    else TextSecurePreferences.setAccessToken(RegistrationActivity.this, null);
    if (!TextUtils.isEmpty(registrationState.profileKey)) TextSecurePreferences.setProfileKey(RegistrationActivity.this, registrationState.profileKey);
    else TextSecurePreferences.setProfileKey(RegistrationActivity.this, null);

    Log.d(TAG, String.format(">>> onCodeComplete (uid:'%d', ufsrvuid:'%s', e164number:'%s', accessToken:'%s', profile_key:'%s'): RECEIVED SIGNON COOKIE: '%s'", registrationState.accountUserid, registrationState.ufsrvUid, registrationState.e164number, registrationState.accessToken, registrationState.profileKey, registrationState.verifiedCookie));
//
    // reset account manager with actual crentials
    accountManager = AccountManagerFactory.createManager(RegistrationActivity.this, registrationState.ufsrvUid, registrationState.password);
    ...
    DatabaseFactory.getIdentityDatabase(RegistrationActivity.this)
                  .saveIdentity(Address.fromSerialized(registrationState.ufsrvUid),
...
//      accountManager.requestVoiceVerificationCode(registrationState.pendingCookie, Locale.getDefault(), registrationState.captchaToken, pushChallenge); //
...
//      }.executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR);
  ...
  private void displayVerificationView(@NonNull String e164number, int callCountdown) {
    if (true||runnableRegoVerificationJob!=null) {// delete check
...
//        title.setText(getString(R.string.RegistrationActivity_enter_the_code_we_sent_to_s, formatNumber(e164number)));
        title.setText(R.string.RegistrationActivity_your_email_is_being_verified);//
        title.clearAnimation();
...
//    subtitle.setText("");
    subtitle.setText(R.string.RegistrationActivity_check_your_email_for_verification_link);//
...
//- this has timeout and will fire off broadcast
...
//    registerReceiver(smsRetrieverReceiver, filter);
  ...
  private void onWrongNumberClicked() {
    ...
    registrationState = new RegistrationState(RegistrationState.State.INITIAL, null, null, Optional.absent(), Optional.absent(), null, null, null, -1, null);
  }
  ...
  private class SmsRetrieverReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
      ...
      } else if (RegistrationStatusVerifierJob.REGO_STATUS_VERIFIED_ACTION.equals(intent.getAction())) {//
  ...
  private class CountryCodeChangedListener implements TextWatcher {
    ...
    public void afterTextChanged(Editable s) {
      ...
      if (!TextUtils.isEmpty(regionCode) && !regionCode.equals("ZZ")) {
        //number.requestFocus(); //- relinquish to email field
      }
    }
...
//        }
  ...
  Runnable setupVerificationStatusChecker (Handler handler)
  {
    Runnable runnableCode = new Runnable() {
      @Override
      public void run () {
        ApplicationContext.getInstance(RegistrationActivity.this)
                .getJobManager()
                .add(new RegistrationStatusVerifierJob());
...
//
  ...
  private static class RegistrationState {
    private enum State {
      INITIAL, VERIFYING, CHECKING, PIN, EXCEPTION // exception
    }
