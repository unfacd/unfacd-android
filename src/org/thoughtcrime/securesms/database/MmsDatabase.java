...
public class MmsDatabase extends MessagingDatabase {
  ...
  public static final String CREATE_TABLE = "CREATE TABLE " + TABLE_NAME + " (" + ID + " INTEGER PRIMARY KEY, "                          +
          ...
          QUOTE_MISSING + " INTEGER DEFAULT 0, " + SHARED_CONTACTS + " TEXT, " + UNIDENTIFIED + " INTEGER DEFAULT 0, " + LINK_PREVIEWS + " TEXT, " +
          REVEAL_DURATION + " INTEGER DEFAULT 0, " + REVEAL_START_TIME + " INTEGER DEFAULT 0, " +
          UFSRV_COMMAND + " TEXT "+//
          ");";
  ...
  private static final String[] MMS_PROJECTION = new String[] {
          ...
          EXPIRES_IN, EXPIRE_STARTED, NOTIFIED, QUOTE_ID, QUOTE_AUTHOR, QUOTE_BODY, QUOTE_ATTACHMENT, QUOTE_MISSING, SHARED_CONTACTS, LINK_PREVIEWS, UNIDENTIFIED, REVEAL_DURATION, REVEAL_START_TIME,
          UFSRV_COMMAND, // ufsrvCommand
          "json_group_array(json_object(" +
                  ...
                  "'" + AttachmentDatabase.UFID + "', " + AttachmentDatabase.TABLE_NAME + "." + AttachmentDatabase.UFID +  //
...
//
  public Reader getGroupUpdateMessages() {
    SQLiteDatabase database = databaseHelper.getReadableDatabase();
  ...
  public Reader getMessageByTimestamp(long timestamp, boolean received) {
    SQLiteDatabase database = databaseHelper.getReadableDatabase();
...
//https://github.com/WhisperSystems/Signal-Android/commit/cdf3a849e924125147508693a6a981d165affafe
...
//  }
  ...
  private List<MarkedMessageInfo> setMessagesRead(String where, String[] arguments) {
    ...
    try {
      cursor = database.query(TABLE_NAME, new String[] {ID, ADDRESS, DATE_SENT, MESSAGE_BOX, EXPIRES_IN, EXPIRE_STARTED, UFSRV_COMMAND}, where, arguments, null, null, null);
...
//          SyncMessageId  syncMessageId  = new SyncMessageId(Address.fromSerialized(cursor.getString(1)), cursor.getLong(2));
  ...
  void trimEntriesForExpiredMessages() {
    ...
    String[] columns = new String[] { AttachmentDatabase.ROW_ID, AttachmentDatabase.UNIQUE_ID, AttachmentDatabase.UFID }; // ufid
    ...
    try (Cursor cursor = database.query(AttachmentDatabase.TABLE_NAME, columns, where, null, null, null, null)) {
      while (cursor != null && cursor.moveToNext()) {
        DatabaseFactory.getAttachmentDatabase(context).deleteAttachment(new AttachmentId(cursor.getLong(0), cursor.getLong(1), cursor.getString(2)));// 3rd arg
      }
    }
  ...
  // update ufsrv body
  public Pair<Long, Long> updateMessageUfsrvCommand(long messageId, String ufsrvCommand) {
    SQLiteDatabase db = databaseHelper.getWritableDatabase();
    ...
    try {
      ...
      if (cursor != null && cursor.moveToNext()) {
        ...
        if (body != null && (Types.isGroupQuit(outboxType) || Types.isGroupUpdate(outboxType))) {
          return new OutgoingGroupMediaMessage(recipient, body, attachments, timestamp, 0, 0, quote, contacts, previews, ufsrvCommandBodyEncoded); // ufsrvbody
        } else if (Types.isExpirationTimerUpdate(outboxType)) {
          return new OutgoingExpirationUpdateMessage(recipient, timestamp, expiresIn, ufsrvCommandBodyEncoded); // ufsrvbody
        }
          ...
          throws MmsException
  {
    ...
    contentValues.put(READ, retrieved.isExpirationUpdate() ? 1 : 0);// doent display expiration messages as unread notification
          ...
          throws MmsException
  {
    ...
    Log.d(TAG, String.format("insertMessageOutbox (MasterSecretUnion): Inserted new OutgoingMediaMessage (messageId:'%d' timestamp:'%d', into threadId:'%d')", messageId, message.getSentTimeMillis(), threadId));
  ...
  public class Reader implements Closeable {
    ...
    private NotificationMmsMessageRecord getNotificationMmsMessageRecord(Cursor cursor) {
      ...
      return new NotificationMmsMessageRecord(id, recipient, recipient,
                                              ...
                                              transactionIdBytes, mailbox, subscriptionId, slideDeck, readReceiptCount, ufsrvCommand);// ufsrv
    }
