...
public class AttachmentDatabase extends Database {
  ...
  private static final String PART_ID_WHERE = ROW_ID + " = ? AND " + UNIQUE_ID + " = ? AND " + UFID + " = ?";

  private static final String[] PROJECTION = new String[] {ROW_ID,
                                                           MMS_ID, CONTENT_TYPE, NAME, CONTENT_DISPOSITION,
                                                           CONTENT_LOCATION, THUMBNAIL, DATA, TRANSFER_STATE,
  ...
  public static final String[] CREATE_INDEXS = {
    ...
    "CREATE INDEX IF NOT EXISTS ufid_index ON " + TABLE_NAME + " (" + UFID + ");", //
  };
  ...
  public @NonNull InputStream getThumbnailStream(@NonNull AttachmentId attachmentId)
      throws IOException
  {
    Log.w(TAG, String.format("getThumbnailStream: attachmentId:'%s'", attachmentId));
          ...
          throws MmsException, IOException
  {
    ...
    contentValues.put(UFID, sourceAttachment.getUfId()); //
  ...
  public void updateAttachmentAfterUpload(@NonNull AttachmentId id, @NonNull Attachment attachment) {
    ...
    values.put(UFID, attachment.getUfId());//
      ...
      if (cursor.getColumnIndex(AttachmentDatabase.ATTACHMENT_JSON_ALIAS) != -1) {
        ...
        for (int i=0;i<array.length();i++) {
          ...
          if (!object.isNull(ROW_ID)) {
            result.add(new DatabaseAttachment(new AttachmentId(object.getLong(ROW_ID), object.getLong(UNIQUE_ID), object.getString(UFID)), // Ufid
                                              ...
                                              : null,
                                              object.getString(object.getString(UFID))));//
          }
        }
        ...
        return Collections.singletonList(new DatabaseAttachment(new AttachmentId(cursor.getLong(cursor.getColumnIndexOrThrow(ROW_ID)),
                                                                                 cursor.getLong(cursor.getColumnIndexOrThrow(UNIQUE_ID)),
                                                                                                cursor.getString(cursor.getColumnIndexOrThrow(UFID))),// ufid
                                                                ...
                                                                : null,
                                                                cursor.getString(cursor.getColumnIndexOrThrow(UFID))));//);
      }
          ...
          throws MmsException
  {
    ...
    contentValues.put(UFID, attachment.getUfId());//
    ...
    AttachmentId attachmentId = new AttachmentId(rowId, uniqueId, attachment.getUfId()); //  attachment.getUfId()
