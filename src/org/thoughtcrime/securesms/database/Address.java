...
import static org.thoughtcrime.securesms.RegistrationActivity.UFSRV_E164NUMBER_COUNTRYCODE;
...
public class Address implements Parcelable, Comparable<Address> {
  ...
  private final UfsrvUid ufsrvId;// this implementation is not complete, still needs integration and coverage
  ...
  //
  private Address(@NonNull String address, UfsrvUid ufsrvId) {
    if (address == null) throw new AssertionError(address);
    this.address = address;
...
//    this(in.readString(), in.readLong());//
  ...
  //
  public static @NonNull Address fromSerialized(@NonNull String serialized, UfsrvUid ufsrvId) {
    return new Address(serialized, ufsrvId);
  }

  public static Address fromExternal(@NonNull Context context, @Nullable String external) {
    if (NumberUtil.isValidEmail(external)) return new Address(external);//
    else return new Address(getExternalAddressFormatter(context).format(external));
  }

  //
  public static Address fromExternal(@NonNull Context context, @Nullable String external, String ufsrvIdEncoded) {
    if (NumberUtil.isValidEmail(external)) return new Address(external, new UfsrvUid(ufsrvIdEncoded));
    else return new Address(getExternalAddressFormatter(context).format(external), new UfsrvUid(ufsrvIdEncoded));
  }

  public static Address fromUfsrvUid(@NonNull UfsrvUid ufsrvId, String address) {
    return new Address(address, ufsrvId);
  }
  ...
  public static @NonNull List<Address> fromSerializedList(@NonNull String serialized, char delimiter) {
    ...
    for (String escapedAddress : escapedAddresses) {
      if (!TextUtils.isEmpty(escapedAddress)) addresses.add(Address.fromSerialized(DelimiterUtil.unescape(escapedAddress, delimiter))); // conditional
    }
  ...
  // not exactly true semantics but will do for now. isPhone cannot be used, as it is multiplexed with ufsrvuid
...
//    return 0;
    if  (address==null) return CONTENTS_FILE_DESCRIPTOR; else return 0;// todo: temporary
  }
  ...
  public static class ExternalAddressFormatter {
    ...
    public String format(@Nullable String number) {
      ...
      if (number.startsWith(UFSRV_E164NUMBER_COUNTRYCODE))            return number;//
