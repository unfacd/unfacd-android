...
// duplicated in IncomingMessageObserver.java. Not in use, but was reinstated in https://github.com/signalapp/Signal-Android/commit/5f31762220fa5ca4a00138e74500bbe4b5232578
...
//  public  final EventsList mEvents = EventsList.getInstance();
  //
...
//    networkRequirementProvider.setListener(this);
    ...
    // broadcast that we have started
  ...
  public IBinder onBind(Intent intent) {
    // because we allow this service to be bound to
    return binder;
    //-
    //return null;
  }
        ...
        if (envelope != null) {
          ...
          if ((envelope.getUfsrvType() == SignalServiceProtos.UfsrvCommandWire.UfsrvType.UFSRV_FENCE_VALUE)) {
            ...
            Log.w(TAG, String.format("(%d, %s) Retrieved envelope! source:'%s', command:'%d', eid:'%d' nFences:'%d', fid:'%d'", this.hashCode(), Thread.currentThread().getId(),
                    envelope.getSource(), envelope.getUfsrvCommand().getUfsrvtype().getNumber(), envelope.getUfsrvCommand().getHeader().getEid(),
...
//            receiveJob.handle(envelope);// doesn't rely on JobManager, calls handle directly
          new PushContentReceiveJob(getApplicationContext()).processEnvelope(envelope); // doesn't rely on JobManager, calls handle directly
