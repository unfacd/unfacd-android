...
public class AvatarImageView extends AvatarWithIndicators/*AppCompatImageView*/ {//
      ...
      if (!photo.equals(recipientContactPhoto)) {
        ...
        if (photo.contactPhoto != null) {
          requestManager.load(photo.contactPhoto)
                  ...
                  //              .signature(new ObjectKey(UfsrvFenceUtils.isAvatarUfsrvIdLoaded(recipient.getGroupAvatarUfsrvId())?recipient.getGroupAvatarUfsrvId():""))//
                  .circleCrop()
                  .into(this);
        } else {
  ...
  // reinstated pre-GlideApp as there are some unresoveld caching issues with ConversationTitleView's avatar
    public void setAvatar(Context context, final @Nullable Recipient recipient, boolean quickContactEnabled) {
      if (recipient != null) {
        GroupDatabase groupDatabase = DatabaseFactory.getGroupDatabase(context);
        Optional<GroupDatabase.GroupRecord> groupRecord   = groupDatabase.getGroup(recipient.getAddress().toGroupString());

        if (groupRecord.isPresent() && groupRecord.get().getAvatar() != null) {
          Bitmap avatarBitmap=BitmapFactory.decodeStream(new ByteArrayInputStream(groupRecord.get().getAvatar()));
          setImageDrawable(new BitmapDrawable(getResources(), avatarBitmap));
        }
