...
public class AttachmentCopyJob extends BaseJob {
  ...
  protected void onRun() throws Exception {
    ...
    for (AttachmentId destinationId : destinationIds) {
      //
      List<DatabaseAttachment> attachmentRecord = database.getAttachmentsForRowId(sourceId.getRowId());
      if (attachmentRecord.size() > 0) {
        AttachmentId sourceIdUpdated = new AttachmentId(sourceId.getRowId(), sourceId.getUniqueId(), attachmentRecord.get(0).getUfId());
        database.copyAttachmentData(sourceIdUpdated, destinationId);
      }
      //
    }
  }
