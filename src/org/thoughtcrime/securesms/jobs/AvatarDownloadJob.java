  ...
  public void onRun() throws IOException {
    ...
    try {
      if (record.isPresent()) {
        ...
        // todo: should go in initialisation and serialisation and constructor
        ...
        SignalServiceAttachmentPointer pointer     = new SignalServiceAttachmentPointer(avatarUfId, avatarId, contentType, key, Optional.of(0), Optional.absent(), 0, 0, digest, fileName, false, Optional.absent());//  ufId
        InputStream                    inputStream = receiver.retrieveAttachment(pointer, attachment, MAX_AVATAR_SIZE);
        Bitmap                         avatar      = BitmapUtil.createScaledBitmap(context, new AttachmentModel(attachment, key, 0, digest), 500, 500);

        database.updateAvatar(encodeId, BitmapUtil.toByteArray(avatar), avatarUfId);//
        inputStream.close();
      }
