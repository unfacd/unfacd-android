...
public class PushMediaSendJob extends PushSendJob {
          ...
          UndeliverableMessageException
  {
    ...
    try {
      ...
      Log.d(TAG, String.format("PushMediaSendJob:deliver(): sending message to '%s'", address.getNumber()));
      ...
      SignalServiceProtos.MessageCommand messageCommand= MessageSender.buildMessageCommandFinal(ApplicationContext.getInstance(),
                                                                                                ...
                                                                                                message.getBody(),
                                                                                                Optional.fromNullable(serviceAttachments),
                                                                                                quote,
      ...
      if (address.getNumber().equals(TextSecurePreferences.getUfsrvUserId(context))) {// usfrv
        ...
        return messageSender.sendMessage(address, UnidentifiedAccessUtil.getAccessFor(context, message.getRecipient()), mediaMessage, new UfsrvCommand(messageCommand, false)).getSuccess().isUnidentified();//
      }
