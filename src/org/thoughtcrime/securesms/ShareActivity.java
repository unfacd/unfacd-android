        ...
        implements ContactSelectionListFragment.OnContactSelectedListener, SwipeRefreshLayout.OnRefreshListener
{
  ...
  public void onContactSelected(String number) {
    ...
    if (!recipient.isGroupRecipient()) {
      //user chose a user contact
      List<Long> groupFids = DatabaseFactory.getGroupDatabase(this).getForTwoGroups(recipient, Recipient.fromUfsrvUid(this, new UfsrvUid(TextSecurePreferences.getUfsrvUserId(this)), false));
      if (groupFids.size()==0) {
        //todo: implement group for two
        Optional<GroupManager.GroupActionResult> result = UfsrvFenceUtils.createGroupForTwo(this, recipient);
        if (result.isPresent()) {
      ...
      recipient = Recipient.fromFid(this, groupFids.get(0), false);
      existingThread = DatabaseFactory.getThreadDatabase(this).getThreadIdIfExistsFor(recipient);
    }
    //
  ...
  //
  private Optional<GroupManager.GroupActionResult> createGroupForTwo (Recipient recipient)
  {
    GroupManager.GroupActionResult result;
    Set<Recipient> members = new HashSet<>();
    members.add(recipient);
    result = GroupManager.createPrivateGroup(this,
                                             members,
                                             null,
                                             false,
