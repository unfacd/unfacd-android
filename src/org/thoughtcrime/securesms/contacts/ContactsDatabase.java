...
public class ContactsDatabase {
  ...
  private static final String CONTACT_MIMETYPE = "vnd.android.cursor.item/vnd.com.unfacd.android.contact";//
  private static final String CALL_MIMETYPE    = "vnd.android.cursor.item/vnd.com.unfacd.android.call";
  private static final String UFSRVUID_MIMETYPE= "vnd.android.cursor.item/vnd.com.unfacd.android.ufsrvuid";
  ...
  public synchronized void setRegisteredUsers(@NonNull Account account,
                                              ...
                                              @NonNull List<ContactTokenDetails> registeredTokenDetailsList, //
                                              boolean remove)
...
//    List<List<Address>>                 registeredChunks     = Util.chunk(registeredAddressList, 50);
    List<List<ContactTokenDetails>>     registeredChunks     = Util.chunk(registeredTokenDetailsList, 50);//

    //
    int indexCounter=0;

    for (List<ContactTokenDetails> registeredChunk : registeredChunks) {// use token instead of addresses to tap into e164 info
      for (ContactTokenDetails registeredToken : registeredChunk) {
        Address registeredAddress = Address.fromSerialized(registeredToken.getE164number()); //
        if (!currentContacts.containsKey(registeredAddress)) {
          ...
          if (systemContactInfo.isPresent()) {
            ...
            addTextSecureRawContact(operations, account, systemContactInfo.get().number,
                                    ...
                                    registeredToken.getUsername(),registeredToken.getUfsrvUid());
          }
        }
  ...
  public @NonNull Cursor queryTextSecureContacts(String filter) {
    ...
    return new ProjectionMappingCursor(cursor, projectionMap,
                                       new Pair<>(LABEL_COLUMN, "unfacd"),// see ref in ContacsAccessor.java
  ...
  private void addTextSecureRawContact(List<ContentProviderOperation> operations,
                                       ...
                                       long aggregateId, String email, String ufsrvUid) // userid,email
  {
    ...
    operations.add(ContentProviderOperation.newInsert(RawContacts.CONTENT_URI)
            ...
            .withValue(RawContacts.SYNC1, ufsrvUid)//e164number) //
    ...
    operations.add(ContentProviderOperation.newInsert(dataUri)
                           ...
                           .withValue(ContactsContract.Data.MIMETYPE, UFSRVUID_MIMETYPE)
                           .withValue(ContactsContract.Data.DATA1, ufsrvUid)
                           .withValue(ContactsContract.Data.DATA2, context.getString(R.string.app_name))
                           .withValue(ContactsContract.Data.DATA3, ufsrvUid)
                           .withYieldAllowed(true)
                           .build());
    //
    ...
    operations.add(ContentProviderOperation.newInsert(dataUri)
            ...
            .withValue(ContactsContract.Data.DATA1, ufsrvUid)//e164number)
            .withValue(ContactsContract.Data.DATA2, context.getString(R.string.app_name))
