...
public class ProfileCipherOutputStream extends DigestingOutputStream {
  ...
  public ProfileCipherOutputStream(OutputStream out, byte[] key, Function<byte[], AlgorithmParameterSpec> parameterProvider) throws IOException, InvalidAlgorithmParameterException { // spec + excep
    super(out);
    try {
      this.cipher = Cipher.getInstance("AES/GCM/NoPadding");

      byte[] nonce  = generateNonce();
      this.cipher.init(Cipher.ENCRYPT_MODE, new SecretKeySpec(key, "AES"), parameterProvider.apply(nonce));
  ...
  // temporary remedy, as some devices want this as opposed to IVParamSpec (huawei). TAG_LENGTH_BIT  = (TAG_LENGTH_BYTES: 16 * 8) = 128
